...
import { createOutput } from '@runic/io'
import { isArraySocket } from '@runic/sockets'
...
export default ({ name, symbol, fn = _.identity, params = {} }) => tile => {
    ...
    const solidifyParams = (rune, inputs) =>
        _.mapIndexed((param, key) => {
            ...
            if (isArraySocket(param))
                return _.pipe(
                    _.map(_.prop('value')),
                    _.cond([
                        [_.pipe(_.length, _.equals(1), _.not), _.identity],
                        [_.pipe(_.first, _.isArray), _.flattenDepth(1)],
                        [_.T, _.identity],
                    ])
                )(connections)
    ...
    return {
        ...
        worker: (rune, originalInputs, outputs) => {
            ...
            const socket = createOutput(value)
