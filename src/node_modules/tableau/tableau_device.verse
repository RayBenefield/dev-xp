...
static_type<public> := class<concrete>(side_effect_settings):
    @editable PerResource<override>:logic = true
    @editable SubResourceID<override>:string = ""
    @editable PerSubResource<override>:logic = false
static_resource<public> := class<concrete>(side_effect_settings):
    @editable PerResource<override>:logic = false
    @editable SubResourceID<override>:string = ""
    @editable PerSubResource<override>:logic = false
...
tableau_device<public> := class(creative_device):
    ...
    OnBegin<override>()<suspends>:void=
        DeviceTags := GetTags()
        if (not DeviceTags.Has[tableau_device_tag{}]). Warning("❌ [{ResourceID}] Device is missing the `tableau_device_tag`")

    GetResources<public>():[]resource= array. resource{ ID := ResourceID }

    GetResID<public>()<computes>:string= ResourceID
    GetSubResID<public>()<transacts>:string= SideEffectSettings.SubResourceID
    GetPerType<public>()<transacts>:?string= if (SideEffectSettings.PerResource?). option{ ResourceID } else. false
    GetPerSubType<public>()<transacts>:?string= if (SideEffectSettings.PerSubResource?). option{ SideEffectSettings.SubResourceID } else. false
    GetSideEffects<public>()<suspends>:[]side_effect= array{}
