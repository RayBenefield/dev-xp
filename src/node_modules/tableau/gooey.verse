...
prop_str<public> := class<concrete>(prop(string)):
    @editable ID<override>:string = ""
    @editable Default<override>:string = ""

    ToVM<override>():gooey_string=
        gooey_string. VM := prop(string){ ID := ID, Default := Default }.ToVM()

prop_num<public> := class<concrete>(prop(num)):
    @editable ID<override>:string = ""
    @editable Default<override>:num = num{}

    ToVM<override>():gooey_num=
        gooey_num. VM := prop(num){ ID := ID, Default := Default }.ToVM()

prop_float<public> := class<concrete>(prop(float)):
    @editable ID<override>:string = ""
    @editable Default<override>:float = 0.0

    ToVM<override>():gooey_float=
        gooey_float. VM := prop(float){ ID := ID, Default := Default }.ToVM()
...
Combine(VM1:gooey(t), VM2:gooey(u), Transform:type{_(:t, :u):v}, ResultVM:gooey(v) where t:type, u:type, v:type)<suspends>:void=
    var First:t = VM1.Get()
...
MapTransformer(VM:gooey(t), Transform:type{_(:t):u}, ResultVM:gooey(u) where t:type, u:type)<suspends>:void=
    var Value:t = VM.Get()

    sync:
        ResultVM.Init(Transform(Value))
        loop:
...
gooey_float<public> := class(gooey(float)):
    ...
    Init<override>(Default:float)<suspends>:void = VM.Init(Default)
    Set<override>(NewValue:float):void = VM.Set(NewValue)
...
gooey_num<public> := class(gooey(num)):
    ...
    Init<override>(Default:num)<suspends>:void = VM.Init(Default)
    Set<override>(NewValue:num):void = VM.Set(NewValue)
...
gooey_bool<public> := class(gooey(logic)):
    ...
    Init<override>(Default:logic)<suspends>:void = VM.Init(Default)
    Set<override>(NewValue:logic):void = VM.Set(NewValue)
