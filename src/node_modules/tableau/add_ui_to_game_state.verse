...
canvas_settings<public> := class<concrete>():
    @editable Interactive<public>:logic = true
...
hud := class():
    ...
    AttachHUD<public>(
        ...
        Canvas := if (HUDSettings.Interactive?). GameState.GetInteractiveCanvas(PlayerID) else. GameState.GetCanvas(PlayerID)
        if (C := Canvas?). C.AddWidget(MakeWidget(Widget, HUDSettings))
