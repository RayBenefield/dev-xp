...
offset_settings<public> := class<concrete>():
...
canvas_settings<public> := class<concrete>():
    ...
    @editable Offsets<public>:offset_settings = offset_settings{}

    GetOffsets<public>():margin=
        Top := case(VerticalAlign):
            vertical_alignment.Top => Offsets.VerticalPadding
            vertical_alignment.Bottom => -Offsets.VerticalPadding
            _ => 0.0
        Left := case(HorizontalAlign):
            horizontal_alignment.Left => Offsets.HorizontalPadding
            horizontal_alignment.Right => -Offsets.HorizontalPadding
            horizontal_alignment.Center => Offsets.HorizontalPadding
            _ => 0.0
        margin:
            Top := Top
            Left := Left
...
hud := class():
    ...
    AttachHUD<public>(
        ...
        if (UI := GetPlayerUI[player[Agent]]):
            Widget := CreateHUD(GameState, ParticipantID)
...
MakeWidget(Widget:widget, Settings:canvas_settings):canvas_slot=
    ...
    XAlign := case(Settings.HorizontalAlign):
        horizontal_alignment.Center => if (Settings.Offsets.HorizontalPadding > 0.0):
            0.0
        else if (Settings.Offsets.HorizontalPadding < 0.0):
            1.0
        else:
            0.5
        _ => XAnchor
