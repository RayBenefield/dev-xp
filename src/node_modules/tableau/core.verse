...
(HP:half_path).TryComplete<public>(ResourceID:string)<transacts>:?prop_path=
    ...
    return false
(HP:half_path).GetResourceKey<public>()<transacts>:string=
    if (RP := HP.ResourcePath?). "({RP.GetKey()})"
    else. (if (R := HP.ResourceID?). "{R}" else. "?")
(HP:half_path).GetPropertyKey<public>()<transacts>:string=
    if (PP := HP.PropertyPath?). "({PP.GetKey()})"
    else. (if (P := HP.PropertyID?). "{P}" else. "?")
(HP:half_path).GetKey<public>()<transacts>:string= HP.GetResourceKey() + "." + HP.GetPropertyKey()
ToString<public>(P:half_path)<transacts>:string= P.GetKey()
ToDebugString<public>(P:half_path)<transacts>:string= Tab("[HALF PATH]") +
