...
transformer<public> := class<concrete>():
    @editable Type<public>:string = ""
    @editable Pay<public>:load = load{}

num_transformer_provider<public> := class():
    var NumTransformers:[string]num_transformer = map{}

    Register<public>(Type:string, NumTransformer:num_transformer):void=
        if (set NumTransformers[Type] = NumTransformer) {}

    Get<public>(Type:string)<decides><transacts>:num_transformer = NumTransformers[Type]

multiply_num_transformer<public> := class(num_transformer):
    FN<override>(First:num, Second:num):num = First * First

num_transformer<public> := interface:
...
comp_float<public> := class<concrete>(comp_prop(float)):
    ...
    @editable Transformers<override>:[]transformer = array{}
