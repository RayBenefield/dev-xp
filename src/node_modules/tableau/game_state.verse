...
game_state<public> := class():
    ...
    AddResource<public>(Resource:resource):void=
        ...
        if (Resource.ID <> ""):
            if. set Resources[Resource.ID] = Resource
    ...
    CreateResource<public>(ID:string, Type:string, PlayerID:string)<suspends>:void=
        if (not Resources[ID], Template := ResourceTemplates[Type]):
            Resource := CreateFromTemplate(ID, Template)

            var DefaultProps:[string]prop = map{}
            for (PropID->Prop:Resource.Props):
                if (DF := Prop.DefaultFrom?):
                    case(Prop.Type):
                        prop_type.Bool =>
                            Value := GetBoolProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_bool. Default := Value
                        prop_type.Integer =>
                            Value := GetIntProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_int. Default := Value
                        prop_type.Float =>
                            Value := GetFloatProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_float. Default := Value
                        prop_type.Num =>
                            Value := GetNumProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_num. Default := Value
                        prop_type.Str =>
                            Value := GetStrProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_str. Default := Value
                        prop_type.Strs =>
                            Value := GetStrsProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_strs. Default := Value
                        prop_type.Agent =>
                            Value := GetAgentProp(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_agent. Default := Value
                        prop_type.Vec2 =>
                            Value := GetVec2Prop(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_vec2. Default := Value
                        prop_type.Vec3 =>
                            Value := GetVec3Prop(ID, DF, PlayerID).Get()
                            option. set DefaultProps[PropID] = prop_vec3. Default := Value
                        prop_type.Unknown => {}

            NewProps := ConcatenateMaps(Resource.Props, DefaultProps)
            ResourceWithDefaults := ReplaceProps(Resource, NewProps)
    ...
    var HPAbilityCache:[string]event_goo = map{}
    var AbilityCache:[string]event_goo = map{}

    GetAbility<public>(PropPath:prop_path, PlayerID:string):event_goo =
        GetAbility(PropPath.ResourceID, PropPath.PropertyID, PlayerID)

    GetAbility<public>(Source:string, HalfPath:half_path, PlayerID:string):event_goo =
        if (PropPath := HalfPath.TryComplete(Source)?). return GetAbility(PropPath, PlayerID)

        Key := "{Source},{HalfPath.GetKey()},{PlayerID}"
        if (Cached := HPAbilityCache[Key]). return Cached

        PropertyID := HalfPath.PropertyID? or ""
        Scoped := scoped_state{ GameState := Self, Source := Source, PropertyID := PropertyID, PlayerID := PlayerID }
        ResourceIDGoo := AwaitResourceGoo(Source, HalfPath, PlayerID)

        Result := EventGoo(SwitchMap(ResourceIDGoo, Scoped.AwaitAbility))
        if. set HPAbilityCache[Key] = Result
        Result

    GetAbility<public>(ResourceID:string, PropertyID:string, PlayerID:string):event_goo=
        if (ResourceID = ""). return EventGoo()

        Key := "{ResourceID},{PropertyID},{PlayerID}"
        Scoped := scoped_state{ GameState := Self, Source := ResourceID, PropertyID := PropertyID, PlayerID := PlayerID }
        if (Resource := Resources[ResourceID]):
            if (Cached := AbilityCache[Key]). return Cached
            HardResult := Resource.GetAbility(PropertyID).ActionFor(Scoped)
            if. set AbilityCache[Key] = HardResult
            return HardResult

        if (Cached := HPAbilityCache[Key]). return Cached

        Result := EventGoo(SwitchMap(AwaitResourceGoo(ResourceID), Scoped.AwaitAbility))
        if. set HPAbilityCache[Key] = Result
        Result

    GetPropType<public>(PropPath:prop_path):prop_type= GetPropType(PropPath.ResourceID, PropPath.PropertyID)
    GetPropType<public>(ResourceID:string, PropertyID:string):prop_type= Resources[ResourceID].GetPropType(PropertyID) or prop_type.Unknown
...
scoped_state<public> := class():
    ...
    AwaitAbility<public>(ResourceID:string):event_goo= GameState.GetAbility(ResourceID, PropertyID, PlayerID)
    ...
    CreateResource<public>(ResourceID:string, Type:string)<suspends>:void= GameState.CreateResource(ResourceID, Type, PlayerID)
    ...
    GetAbility<public>(ResourceID:string, TargetPropertyID:string):event_goo= GameState.GetAbility(ResourceID, TargetPropertyID, PlayerID)
    AttachAction<public>(ResourceID:string, AbilityID:string, Event:awaitable(t) where t:type):void=
        ActionGoo := GameState.GetAbility(ResourceID, AbilityID, PlayerID)
        spawn. ActionTrigger(Event, ActionGoo)
    AttachAction<public>(AbilityID:string, Event:awaitable(t) where t:type):void=
        ActionGoo := GameState.GetAbility(Source, AbilityID, PlayerID)
        spawn. ActionTrigger(Event, ActionGoo)
