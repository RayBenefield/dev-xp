...
game_state<public> := class():
    ...
    GetInteractiveCanvas<public>(PlayerID:string):?canvas=
        if (C := InteractiveCanvas[PlayerID]). return option. C

        Agent := GetAgentProp(PlayerID, "Possessed By", "").Latest
    ...
    GetCanvas<public>(PlayerID:string):?canvas=
    ...
    AddResource<public>(InitialResource:resource)<suspends>:?resource_vm=
        ...
        for (ID->P:InitialResource.Props). if. set PropTypes[InitialResource.ID][ID] = P.GetType()
    ...
    var ResourceGoo:[string]bool_goo = map{}
    AwaitResourceGoo<public>(ID:string):bool_goo=
        if (Cached := ResourceGoo[ID]). return Cached
        Result := if (Resources[ID]):
            BoolGoo(true)
        else:
            ExistsVM := BoolGoo(false)
            spawn. TriggerExists(ID, ExistsVM)
            ExistsVM
        if. set ResourceGoo[ID] = Result
        Result

    TriggerExists(ID:string, VM:bool_goo)<suspends>:void=
        loop:
