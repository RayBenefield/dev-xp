...
game_state<public> := class():
    ...
    GetBoolProp<public>(Source:string, HalfPath:half_path, PlayerID:string):bool_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ...
            return GetBoolProp(PropPath, SubID)
    ...
    GetIntProp<public>(Source:string, HalfPath:half_path, PlayerID:string):int_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetIntProp(PropPath, SubID)
    ...
    GetFloatProp<public>(Source:string, HalfPath:half_path, PlayerID:string):float_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetFloatProp(PropPath, SubID)
    ...
    GetNumProp<public>(Source:string, HalfPath:half_path, PlayerID:string):num_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetNumProp(PropPath, SubID)
    ...
    GetStrProp<public>(PropPath:prop_path, PlayerID:string):str_goo =
    ...
    GetStrProp<public>(Source:string, HalfPath:half_path, PlayerID:string):str_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetStrProp(PropPath, SubID)
    ...
    GetStrsProp<public>(Source:string, HalfPath:half_path, PlayerID:string):strs_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetStrsProp(PropPath, SubID)
    ...
    GetAgentProp<public>(Source:string, HalfPath:half_path, PlayerID:string):agent_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetAgentProp(PropPath, SubID)
    ...
    GetVec2Prop<public>(Source:string, HalfPath:half_path, PlayerID:string):vec2_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetVec2Prop(PropPath, SubID)
    ...
    GetVec3Prop<public>(Source:string, HalfPath:half_path, PlayerID:string):vec3_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetVec3Prop(PropPath, SubID)
    ...
    GetAction<public>(Source:string, HalfPath:half_path, PlayerID:string):event_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
            return GetAction(PropPath, SubID)
...
scoped_state<public> := class():
    ...
    SetBool<public>(HP:half_path, Value:logic)<suspends>:void= GameState.GetBoolProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetInt<public>(HP:half_path, Value:int)<suspends>:void= GameState.GetIntProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetFloat<public>(HP:half_path, Value:float)<suspends>:void= GameState.GetFloatProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetNum<public>(HP:half_path, Value:num)<suspends>:void= GameState.GetNumProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetStr<public>(HP:half_path, Value:string)<suspends>:void= GameState.GetStrProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetStrs<public>(HP:half_path, Value:[]string)<suspends>:void= GameState.GetStrsProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetAgent<public>(HP:half_path, Value:?agent)<suspends>:void= GameState.GetAgentProp(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetVec2<public>(HP:half_path, Value:vector2)<suspends>:void= GameState.GetVec2Prop(ResolveHalfPath(HP), PlayerID).Update(Value)
    SetVec3<public>(HP:half_path, Value:vector3)<suspends>:void= GameState.GetVec3Prop(ResolveHalfPath(HP), PlayerID).Update(Value)
