...
game_state<public> := class():
    ...
    AwaitResourceGoo<public>(Source:string, HalfPath:half_path, PlayerID:string):str_goo=
        PropertyID := HalfPath.PropertyID? or ""
        ResourceID := if(R := HalfPath.ResourceID?, R <> ""). R else. Source
    ...
    GetStrProp<public>(Source:string, HalfPath:half_path, PlayerID:string):str_goo =
        if (PropPath := HalfPath.TryComplete(Source, PlayerID)?):
            ResourceID := PropPath.ResourceID
            SubID := if(IsRelated[Source, ResourceID]). Source else. PlayerID
    ...
    GetStrProp<public>(ResourceID:string, PropertyID:string, PlayerID:string):str_goo=
...
scoped_state<public> := class():
    ...
    AwaitAction<public>(ResourceID:string):event_goo=
        ...
        GameState.GetAction(ResourceID, PropertyID, PlayerID)
    AwaitBool<public>(ResourceID:string):bool_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetBoolProp(ResourceID, PropertyID, PlayerID)
    AwaitInt<public>(ResourceID:string):int_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetIntProp(ResourceID, PropertyID, SubID)
    AwaitFloat<public>(ResourceID:string):float_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetFloatProp(ResourceID, PropertyID, PlayerID)
    AwaitNum<public>(ResourceID:string):num_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetNumProp(ResourceID, PropertyID, PlayerID)
    AwaitStr<public>(ResourceID:string):str_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetStrProp(ResourceID, PropertyID, PlayerID)
    AwaitStrs<public>(ResourceID:string):strs_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetStrsProp(ResourceID, PropertyID, PlayerID)
    AwaitAgent<public>(ResourceID:string):agent_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetAgentProp(ResourceID, PropertyID, PlayerID)
    AwaitVec2<public>(ResourceID:string):vec2_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetVec2Prop(ResourceID, PropertyID, PlayerID)
    AwaitVec3<public>(ResourceID:string):vec3_goo=
        SubID := if(GameState.IsRelated[Source, ResourceID]). Source else. PlayerID
        GameState.GetVec3Prop(ResourceID, PropertyID, PlayerID)

    ToResourceID<public>(HP:half_path):str_goo= GameState.AwaitResourceGoo(Source, HP, PlayerID)
    ...
    ToStr<public>(HP:half_path):str_goo= GameState.GetStrProp(Source, HP, PlayerID)
    ...
    SetInt<public>(HP:half_path, Value:int)<suspends>:void=
        PP := ResolveHalfPath(HP)
        GameState.GetIntProp(PP.ResourceID, PP.PropertyID, PlayerID).Update(Value)
    ...
    SetStr<public>(HP:half_path, Value:string)<suspends>:void=
        PP := ResolveHalfPath(HP)
        GameState.GetStrProp(PP.ResourceID, PP.PropertyID, PlayerID).Update(Value)
    ...
    GetStr<public>(TargetPropertyID:string):str_goo= GameState.GetStrProp(Source, TargetPropertyID, PlayerID)
