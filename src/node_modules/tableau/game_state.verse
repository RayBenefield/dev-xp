...
game_state<public> := class():
    ...
    AddResource<public>(InitialResource:resource, Buy:effect)<suspends>:resource_vm=
        ID := InitialResource.ID
        VM := resource_vm{ ID := ID, BuyEffect := Buy }
    ...
    AddProducer<public>(InitialProducer:producer, Buy:effect)<suspends>:producer_vm=
        ID := InitialProducer.ID
        VM := producer_vm{ ID := ID, BuyEffect := Buy }
