...
CreateBasicResource<public><constructor>(Name:string) := resource:
    ...
    StaticProperties := array:
        property_str{ ID := "Name", Default := Name }
    Properties := array:
        property_num{ ID := "Count", Default := num{} }
...
CreateProducerResource<public><constructor>(Name:string) := resource:
    ...
    Abilities := array:
        ability:
            ...
            Effects := array:
                ...
                effect{ Type := "Increase", Payload := payload. Float := 100.0 }
    ...
    #     ability{ ID := "Produce" }
    #         effector{ FN := "Add Resources" }
    # ComputedProperties := array:
    #     property{ ID := "Cost" }
    #         PropertyID := "Cost Growth", FN := "Identity"
    #         PropertyID := "Count", FN := "ToFloat"
    #         PropertyID := "Count", FN := "Power"
    #         PropertyID := "Base Cost", FN := "Multiply"
    #     property{ ID := "Production Amount" }
    #         PropertyID := "Count", FN := "Power"
    #         PropertyID := "Production", FN := "Multiply"
    # Triggers := array:
...
game_device := class(creative_device):
    ...
    OnBegin<override>()<suspends>:void=
        ...
        GameState := game_state. EffectorProvider := EffectorProvider

        GetPropertyEffector := get_property_effector. GameState := GameState
        EffectorProvider.Register("Get Property", GetPropertyEffector)
        IncreaseEffector := increase_effector. GameState := GameState
        EffectorProvider.Register("Increase", IncreaseEffector)
        SetPropertyEffector:= set_property_effector. GameState := GameState
        EffectorProvider.Register("Set Property", SetPropertyEffector)
        ChangeSubjectEffector:= change_subject_effector. GameState := GameState
        EffectorProvider.Register("Change Subject", ChangeSubjectEffector)
        ...
        spawn. PseudoController(Dispatcher, ResourceRepository, SetPropertyEffector)
