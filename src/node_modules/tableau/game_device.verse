...
CreateBasicResource<public><constructor>(Name:string) := resource:
    ...
    Abilities := array:
        ability:
            ID := "Increase Count"
            Effects := array:
                effect{ FN := "Increase", Pay := load. Str := "Count" }
    ...
    CompProps := comp_props:
        Nums := array:
            comp_num:
                ...
                BaseProp := "Count"
                Computer := computer{ FN := "Multiply", Pay := load. Str := "Production" }
            comp_num:
                ID := "Cost Rate"
                BaseProp := "Cost Growth"
                Computer := computer{ FN := "Power", Pay := load. Str := "Count" }
            comp_num:
                ID := "Cost"
                BaseProp := "Cost Rate"
                Computer := computer{ FN := "Multiply", Pay := load. Str := "Base Cost" }
        Bools := array:
            comp_bool:
                ID := "Can Afford"
                BaseResource := "Gold"
                BaseProp := "Count"
                Computer := computer{ FN := ">=", Pay := load. Str := "Cost" }
    Abilities := array:
        ability:
            ID := "Buy"
            Effects := array:
                effect{ FN := "Is True", Pay := load. Str := "Can Afford" }
...
game_device := class(creative_device):
    ...
    ReadProperty(GameState:game_state)<suspends>:void=
        PropID := "Can Afford"
        if (Prop := GameState.Resources["Merchant"].CompProps.Bools[PropID]):
            FinalProp := Prop.ValueFor("1")
            loop:
                Result := FinalProp.UpdatedEvent().Await()
                if (Result?). Print("{PropID}: True") else. Print("{PropID}: False")
