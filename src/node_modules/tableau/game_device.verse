...
CreateBasicResource<public><constructor>(Name:string) := resource:
    ...
    Abilities := array:
        ability:
            ID := "Increase Count"
            Effects := array:
                effect{ Type := "Increase", Payload := payload. Str := "Count" }
                effect{ Type := "Set Property", Payload := payload. Str := "Count" }

CreateProducerResource<public><constructor>(Name:string) := resource:
    ID := Name
    StatProps := props:
        Strs := array:
            prop_str{ ID := "Name", Default := Name }
        Floats := array:
            prop_float{ ID := "Cost Growth", Default := 1.07 }
            prop_float{ ID := "Base Cost", Default := 3.738 }
    ...
    Props := props:
        Nums := array:
            prop_num{ ID := "Count", Default := num{} }
        Floats := array:
            prop_float{ ID := "Production", Default := 1.67 }
    ...
    CompProps := comp_props:
        Nums := array:
            comp_num:
                ...
                BaseProp := "Count"
                Transformers := array:
                    transformer{ Type := "Multiply", Payload := payload. Str := "Production" }
            comp_num:
                ID := "Cost"
                BaseProp := "Count"
                Transformers := array:
                    transformer{ Type := "Reverse Power", Payload := payload. Str := "Cost Growth" }
                    transformer{ Type := "Multiply", Payload := payload. Str := "Base Cost" }
        # Bools := array:
        #     comp_bool:
        #         ID := "Can Afford"
        #         BaseResource := "Gold"
        #         BaseProp := "Count"
        #         Transformer := transformer{ Type := ">=", Payload := payload. Str := "Cost" }
    Abilities := array:
        ability:
            ID := "Buy"
            Effects := array:
                effect{ Type := "Get Property", Payload := payload. Str := "Count" }
                effect{ Type := "Increase", Payload := payload. Float := 1.0 }
                effect{ Type := "Set Property", Payload := payload. Str := "Count" }
    ...
    #             effector{ FN := "Pay Cost" }
        ability:
            ID := "Produce"
            Effects := array:
                effect{ Type := "Get Property", Payload := payload. Str := "Production Amount" }
                effect{ Type := "Change Subject", Payload := payload. Str := "Gold" }
                effect{ Type := "Execute Ability", Payload := payload. Str := "Increase Count" }
    # Triggers := array:
    #     trigger{ ID := "Produce Timer", Type := "Timer" }
    #     trigger{ ID := "Buy Zone", Type := "Proximity" }
    # Checks := array:
    #     check{ ID := "Owned By" }
    #     check{ ID := "Is Maxed" }
...
game_device := class(creative_device):
    ...
    OnBegin<override>()<suspends>:void=
        ...
        ResourceRepository:resource_repository = resource_repository:
            Initial := array:
                ...
                CreateProducerResource("Merchant")
                CreateProducerResource("Weaponsmith")
                CreateProducerResource("Armorsmith")

        Dispatcher:dispatcher = dispatcher{}


        EffectorProvider := effector_provider{}
        NumTransformerProvider := num_transformer_provider{}
        GameState := game_state:
            EffectorProvider := EffectorProvider
            NumTransformerProvider := NumTransformerProvider

        Effects:[string]effector = map:
            "Get Property" => get_property_effector. GameState := GameState
            "Increase" => increase_effector. GameState := GameState
        ...
        NumTransformers:[string]num_transformer = map:
            "Multiply" => multiply_num_transformer{}

        for (Name->NumTransformer:NumTransformers). NumTransformerProvider.Register(Name, NumTransformer)
        ...
        if (Effect := set_property_effector[Effects["Set Property"]]):
            spawn. PseudoController(Dispatcher, ResourceRepository, Effect)
    ...
    PseudoController(Dispatcher:dispatcher, Repo:resource_repository, SetPropertyEffector:set_property_effector)<suspends>:void=
        loop:
            Command := SetPropertyEffector.SetPropertyCommand.Await()
            ResourceID := Command.Subject
            PropertyID := Command.Property
            ParticipantID := Command.Target
            Value := Command.Payload.Num
