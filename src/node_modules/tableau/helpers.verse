...
(GameState:game_state).TriggerAction<public>(PropSource:prop_path, PlayerID:string, ActionSource:prop_path, TestValue:logic)<suspends>:void=
    ActionGoo := GameState.GetAction(ActionSource.ResourceID, ActionSource.PropertyID, PlayerID)
    Prop := GameState.GetBoolProp(PropSource.ResourceID, PropSource.PropertyID, PlayerID)
    var Values:[]logic = array. Prop.Get()
    sync:
        loop:
        ...
        loop:
            loop:
                if (Value := Values[0]):
                    if(Value = TestValue). ActionGoo.Set()
                    if (NewValues := Values.Slice[1]). set Values = NewValues
                else. break
            NewValue := Prop.UpdatedEvent().Await()
