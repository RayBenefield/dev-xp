...
(GameState:game_state).SyncNum<public>(PropSource:prop_path, PlayerID:string, Tap:type{_(:num)<suspends>:void})<suspends>:void=
    Prop := GameState.GetNumProp(PropSource.ResourceID, PropSource.PropertyID, PlayerID)
    Tap(Prop.Get())
    loop:
        Value := Prop.UpdatedEvent().Await()
        spawn. Tap(Value)
...
(GameState:game_state).SyncVec2<public>(PropSource:prop_path, PlayerID:string, Tap:type{_(:vector2)<suspends>:void})<suspends>:void=
    Prop := GameState.GetVec2Prop(PropSource.ResourceID, PropSource.PropertyID, PlayerID)
    Tap(Prop.Get())
    loop:
        Value := Prop.UpdatedEvent().Await()
        spawn. Tap(Value)
