...
(GameState:game_state).AttachArgAction<public>(PropSource:prop_path, PlayerID:string, Event:awaitable(prop_path))<suspends>:void=
    ActionGoo := GameState.GetAction(PropSource.ResourceID, PropSource.PropertyID, PlayerID)
    spawn. WatchArgAction(Event, ActionGoo)

WatchArgAction(Event:awaitable(prop_path), ActionGoo:event_goo)<suspends>:void=
    loop:
        Arg := Event.Await()
        ActionGoo.Update(Arg)
