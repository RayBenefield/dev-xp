...
MapRace(Array:[]t, Function(Element:t)<suspends>:u where t:type, u:type)<suspends>:u=
    if(Array.Length > 2):
...
MapSync(Array:[]t, Function(Element:t)<suspends>:u where t:type, u:type)<suspends>:[]u=
    if(Array.Length > 2):
        MedianElement := Floor(Array.Length/2 or Err())
        LeftArray := Array.Slice[0, MedianElement] or Err()
        RightArray := Array.Slice[MedianElement] or Err()
...
(Number:float).ToInt():int = if (Num := Floor[Number]). Num else. 0