...
const binConfigs = ({ packages, dependencies }) =>
    packages
        .filter(p => p.bin)
...
const mainConfigs = ({ packages, dependencies }) =>
    packages
        .filter(p => !p.bin)
        .map(p => ({
            ...p,
...
export default ({ packages = [], dependencies } = {}) =>
    new Promise(res =>
        res(
            [].concat(
                ...binConfigs({ packages, dependencies }),
                ...mainConfigs({ packages, dependencies }),
            ),
        ),
    );
