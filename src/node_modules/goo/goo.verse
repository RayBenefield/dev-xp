...
SwitchMap<public>(VM:goo(t), Finder:type{_(:t):goo(u)} where t:type, u:type):goo(u)=
    ResultVM:goo(u) = goo(u){}
    spawn. Switcher(VM, Finder, ResultVM)
    return ResultVM

Switcher<public>(VM:goo(t), Finder:type{_(:t):goo(u)}, ResultVM:goo(u) where t:type, u:type)<suspends>:void=
    var Key:t = VM.Get()
    var Value:u = Finder(Key).Get()

    spawn. ResultVM.Init(Value)
    loop:
        race:
            block:
                set Value = Finder(Key).Get()
                ResultVM.Set(Value)
                loop:
                    set Value = Finder(Key).UpdatedEvent().Await()
                    ResultVM.Set(Value)
            block:
...
vec3_goo<public> := class(goo(vector3)):
    ...
    var Latest<public>:vector3 = vector3{}
