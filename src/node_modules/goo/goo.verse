...
Entwine<public>(VM1:goo(t), VM2:goo(u), VM3:goo(v), Transform:type{_(:t, :u, :v):w} where t:type, u:type, v:type, w:type):goo(w)=
    ResultVM:goo(w) = goo(w){}
    spawn. Combine3(VM1, VM2, VM3, Transform, ResultVM)
    return ResultVM
Combine3<public>(VM1:goo(t), VM2:goo(u), VM3:goo(v), Transform:type{_(:t, :u, :v):w}, ResultVM:goo(w) where t:type, u:type, v:type, w:type)<suspends>:void=
    var First:t = VM1.Get()
    var Second:u = VM2.Get()
    var Third:v = VM3.Get()
    UpdatedEvent := event(){}

    spawn. ResultVM.Init(Transform(First, Second, Third))
    sync:
        loop:
            set First = VM1.UpdatedEvent().Await()
            UpdatedEvent.Signal()
        loop:
            ...
            UpdatedEvent.Signal()
        loop:
            set Third = VM3.UpdatedEvent().Await()
            UpdatedEvent.Signal()
        loop:
            UpdatedEvent.Await()
            Transformed := Transform(First, Second, Third)
            ResultVM.Update(Transformed)
...
event_goo<public> := class(goo(prop_path)):
    Fallback<override>:?prop_path = option. prop_path{}
...
str_goo<public> := class(goo(string)):
    Fallback<override>:?string = option. ""
...
strs_goo<public> := class(goo([]string)):
    Fallback<override>:?[]string = option. array{}
...
int_goo<public> := class(goo(int)):
    Fallback<override>:?int = option. 0
...
float_goo<public> := class(goo(float)):
    Fallback<override>:?float = option. 0.0
...
num_goo<public> := class(goo(num)):
    Fallback<override>:?num = option. num{}
...
bool_goo<public> := class(goo(logic)):
    Fallback<override>:?logic = option. false
...
vec2_goo<public> := class(goo(vector2)):
    Fallback<override>:?vector2 = option. vector2{}
...
vec3_goo<public> := class(goo(vector3)):
    Fallback<override>:?vector3 = option. vector3{}
