...
import chooseWinners from '@fever/choose-winners'
import { tap, filter, map, mergeMap, withLatestFrom } from 'rxjs/operators'

export default ({ moderator, db, logger: baseLogger }) => {
    const logger = baseLogger.child({ plugin: '@fever/pick' })
    ...
    moderator
        ...
        .pipe(
            ...
            map(([allEntries, winners, repo]) => {
                const noWinners = omitBy(allEntries, ({ id }) => id in winners)
                const makeEntries = ({ name, entries }) => ({ name, entries })
