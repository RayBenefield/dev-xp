...
export default ({ config: initialConfig, renderer, style, decide } = {}) => {
    const { name = '???', plugins: unkeyed = [], plugs = {} } = initialConfig
    ...
    const config = {
        ...
        plug: fn => data => {
            ...
            // Let the devs know they need to add the respective plug to their tool
            if (!(fn in plugs)) throw Error($.noPlug(name, fn))
    ...
    return (unparsed = ['']) => {
        ...
        const finalConfig = { ...config, ...newArgs }
