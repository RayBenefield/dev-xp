...
slot_generator := class():
    GenerateResourceEvent<public>:event(resource) = event(resource){}
    Init<public>():void=
        GridCount := 25
        for (Index := 0..GridCount-1). GenerateResourceEvent.Signal(GenerateSlotResource(Index))
...
GenerateSlotResource<constructor>(Index:int)<transacts> := resource:
    ID := "Slot{Index+1}"
    Type := "Slot"
    Props := map:
...
slot_creator_device<public> := class(tableau_device):
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        SlotGenerator := slot_generator{}
        array:
            Generate(SlotGenerator.GenerateResourceEvent)
            StartWith(SlotGenerator.Init)
