...
CreateSlotResource<constructor>(Offset:vector3, GridSize:float, MaxX:int, MaxY:int, Index:int) := resource:
    ID := "Slot{Index+1}"
    Type := "Slot"
    Props := map:
...
slot_device<public> := class(tableau_device):
    ...
    GetResources<override>():[]resource=
        ...
        for (Index := 0..GridCount-1):
            CreateSlotResource(Offset, GridSize, MaxX, MaxY, Index)
