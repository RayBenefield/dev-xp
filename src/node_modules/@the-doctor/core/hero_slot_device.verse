...
hero_slot_device<public> := class(tableau_device):
    ...
    GetSideEffects<override>()<suspends>:[]side_effect=
        ...
        array:
            ...
            PushFloat("Health Ratio", SetScaleY(HealthBarProp).FN)
            PushVec3("Location", Move(AvatarProp, 0.5, V3X(-0.3)).FN)
            PushVec3("Location", Move(AvatarFrameProp, 0.5, V3X(-0.2)).FN)
            PushVec3("Location", Move(AvatarBackgroundProp, 0.5, V3X(-0.1)).FN)
            PushVec3("Location", Move(HealthBarProp, 0.5, V3(-0.5, -48.0, -48.0)).FN)
            PushVec3("Location", Move(HealthBackgroundProp, 0.5, V3(-0.4, -48.0, -48.0)).FN)
            PushVec3("Location", Move(HealthFrameProp, 0.5, V3(-0.6, 0.0, -48.0)).FN)
