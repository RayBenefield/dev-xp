...
# REQUEST: Currently not possible due to "References to overloaded, super qualified, and intrinsic functions are not yet implemented."
# CombineAll<public>(Mappables:[][string]t where t:type):[string]t= Mappables.Reduce(ConcatenateMaps, map{})

Combine<public><constructor>(SourceMap:[string]t where t:type) := map_combiner(t). SourceMap := SourceMap
map_combiner<public>(t:type) := class():
    SourceMap<public>:[string]t
    WithAll<public>(NewMap:[string]t):[string]t= ConcatenateMaps(SourceMap, NewMap)

CreateMap<public>(Mappable:[]tuple(string, t) where t:type):[string]t=
    var Result:[string]t = map{}
    for (Entry:Mappable, Key := Entry(0), Value := Entry(1)):
        if. set Result[Key] = Value
    Result
