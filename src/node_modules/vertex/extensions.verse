...
(Awaitables:[]awaitable(t) where t:type).Sync<public>()<suspends>:[]t= MapSync(Awaitables, JustAwait)
...
(Awaitable:awaitable(t) where t:type).Debounce<public>(Interval:float)<suspends>:void=
    var Interrupted:logic = false
    loop:
        set Interrupted = race:
            block:
                Awaitable.Await()
                true
            block:
                Sleep(Interval)
                false
        if(not Interrupted?). break

FilterOptions<public>(Arr:[]?t where t:type)<computes>:[]t= for(Element:Arr, Value := Element?). Value
...
(Array:[]t where t:type).Partition<public>(Predicate:type{_(:t)<decides><transacts>:void})<transacts>:[][]t=
    var Trues:[]t = array{}
    var Falses:[]t = array{}
    for(Element:Array):
        if(Predicate[Element]). set Trues += array. Element
        else. set Falses += array. Element
    array{ Trues, Falses }
...
(Map:[t]u where t:subtype(comparable), u:type).OnlyValues<public>():[]u= for(V:Map). V
...
CombineAllArrays<public>(Arrays:[][]t where t:type)<transacts>:[]t= Arrays.Reduce(ReverseConcatenate, array{})
...
CreateMap<public>(Mappable:[]tuple(t, u) where t:subtype(comparable), u:type)<transacts>:[t]u=
...
(Awaitable:awaitable(t) where t:subtype(comparable)).AwaitFor<public>(TargetValue:t)<suspends>:t =
    loop:
        SignaledValue := Awaitable.Await()
        if(SignaledValue = TargetValue). return SignaledValue
...
Spawn<public>(Asset:?creative_prop_asset, Translation:vector3):?creative_prop = if (A := Asset?). Spawn(A, Translation, R3(), V3One(), option. material[BLANK{}]) else. false
...
TeleportTo<public>(Prop:?creative_prop, Location:vector3):void= if (P := Prop?):
    if. P.TeleportTo[Location, P.GetTransform().Rotation]
...
V2<public>(X:int, Y:int)<computes>:vector2= vector2{ X := X*1.0, Y := Y*1.0 }
...
ToString<public>(Transform:transform):string= Tab("[TRANSFORM]") +
    WithIndent("Translation: {Transform.Translation}") +
    WithIndent("Rotation: {Transform.Rotation.ToVector3()}") +
    WithIndent("Scale: {Transform.Scale}") +
    EndTab()
...
(Vector3:vector3).Equals<public>(Other:vector3)<decides><transacts>:void=
    IsAlmostEqual[Vector3, Other, 0.001]
(Value:float).AngleEquals<public>(Other:float)<decides><transacts>:void=
    IsAlmostEqual[Value, Other, 1.0]

(Rotation:rotation).ToVector3<public>()<transacts>:vector3= Rotation.GetYawPitchRollDegrees().ToVector3()
(Rotation:rotation).Equals<public>(Other:rotation)<decides><transacts>:void=
