    ...
    it('should return a single project', assert =>
        findPackages(root('one-named-package'))
            .then(result => assert.deepEqual(result, [{
                ...
                dir: resolve(
                    __dirname,
                    'fixtures/one-named-package/src/node_modules/module'
                ),
            }]))
    );

    it('should return multiple projects with depth', assert =>
        findPackages(root('multiple-packages'))
            .then((result) => {
                ...
                assert.ok(
                    ...
                    && dirs.includes(resolve(
                        __dirname,
                        'fixtures/multiple-packages/src/node_modules/module'
                    ))
                    && dirs.includes(resolve(
                        __dirname,
                        'fixtures/multiple-packages/src/node_modules/@module/test'
                    ))
                    && dirs.includes(resolve(
                        __dirname,
                        'fixtures/multiple-packages/src/node_modules/@module/another'
                    ))
                );
            })
    );
