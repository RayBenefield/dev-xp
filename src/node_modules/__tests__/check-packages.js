...
const packages = dir => ({
    packages: [
        {
            dir: resolve(__dirname, `fixtures/modules/${dir}`),
            // eslint-disable-next-line global-require, import/no-dynamic-require
            ...require(resolve(
                __dirname,
                `fixtures/modules/${dir}/package.json`,
            )),
        },
    ],
});
    ...
    it => {
        ...
        it('should return a failing check for a package with no main file', assert =>
            checkPackages(packages('no-main')).then(result =>
                assert.deepEqual(result, [
                    {
                        dir: resolve(__dirname, 'fixtures/modules/no-main'),
                        name: 'no-main',
                        publishable: false,
                    },
                ]),
                ...
                assert.deepEqual(result, [
                    {
                        dir: resolve(
                            __dirname,
                            'fixtures/modules/main-index-js-readme',
                        ),
        ...
        it('should return a failing check for a package with a defined bin cli.js, but no readme', assert =>
            checkPackages(packages('bin-cli-js')).then(result =>
                assert.deepEqual(result, [
                    {
                        dir: resolve(__dirname, 'fixtures/modules/bin-cli-js'),
                        name: 'bin-cli-js',
                        bin: {
                            cli: 'cli.js',
                        },
                ...
                assert.deepEqual(result, [
                    {
                        dir: resolve(
                            __dirname,
                            'fixtures/modules/bin-cli-js-readme',
                        ),
