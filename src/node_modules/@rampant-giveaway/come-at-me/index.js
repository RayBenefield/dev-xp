...
export default ({
    ...
    throttledEntries$.subscribe(({ friend, entrant }) => {
        ...
        const poolRef = database.doc(`pools/${prefix}`)
        const entrantRef = database.doc(`pools/${prefix}/entries/${entrant.id}`)
        database.runTransaction(t =>
            t.getAll(poolRef, entrantRef).then(([poolDoc, entrantDoc]) => {
                const poolData = poolDoc.data()
                const entrantData = entrantDoc.data()

                if (!poolData) {
                    t.set(poolRef, { entries: 1 })
                    t.set(entrantRef, {
                        name: entrant.name,
                        entries: 1,
                ...
                if (!entrantData) {
                    t.update(poolRef, { entries: poolData.entries + 1 })
                    t.set(entrantRef, {
                        name: entrant.name,
                        entries: 1,
                        [friend.id]: { entries: 1, name: friend.name },
                ...
                t.update(entrantRef, {
                    name: entrant.name,
                    entries: entrantData.entries + 1,
                    [friend.id]: {
                        entries: entrantData[friend.id]
                            ? entrantData[friend.id].entries + 1
                            : 1,
                        name: friend.name,
                    },
                })
