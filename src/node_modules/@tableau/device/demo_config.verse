    ...
    GetResources<override>():[]resource= array:
        ...
        resource:
            ...
            Props := map:
                ...
                "Has Wood" => prop_bool{ Focus := Path("Count"), FN := ">=", Pay := Load(Path("ONE")) }
                "Can Afford" => prop_bool{ Focus := Path("Gold", "Count"), FN := ">=", Pay := Load(Path("Cost")) }
                "With Bonus" => prop_num{ Focus := Path("Harvest Rate"), FN := "Multiply", Pay := Load(Path("Bonus")) }
            Abilities := map:
                "Harvest" => array:
