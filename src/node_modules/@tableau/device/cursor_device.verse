...
cursor_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        if (Ability := State.GameState.Resources[ResourceID].Abilities["Move Forward"]):
            Forward.ReleasedEvent.Subscribe(Ability.ActionFor(State.PlayerID).ExecuteAgentFloat)
        if (Ability := State.GameState.Resources[ResourceID].Abilities["Move Left"]):
            Left.ReleasedEvent.Subscribe(Ability.ActionFor(State.PlayerID).ExecuteAgentFloat)
        if (Ability := State.GameState.Resources[ResourceID].Abilities["Move Right"]):
            Right.ReleasedEvent.Subscribe(Ability.ActionFor(State.PlayerID).ExecuteAgentFloat)
        if (Ability := State.GameState.Resources[ResourceID].Abilities["Move Back"]):
            Back.ReleasedEvent.Subscribe(Ability.ActionFor(State.PlayerID).ExecuteAgentFloat)
