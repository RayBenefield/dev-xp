...
cursor_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    @editable Cursor:?creative_prop = false
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        State.AttachAction("Move Forward", Forward.ReleasedEvent)
        State.AttachAction("Move Left", Left.ReleasedEvent)
        State.AttachAction("Move Right", Right.ReleasedEvent)
        State.AttachAction("Move Back", Back.ReleasedEvent)

        array:
            move{ PropertyID := "Location", Prop := Cursor, Time := 0.2 }
