...
spawn_vfx_device<public> := class(creative_device, tableau_device):
    ...
    Init<override>(GameState:game_state)<suspends>:void=
        Watcher := player_watcher{ GameState := GameState, OnPossessed := HandleVFX }
        spawn. Watcher.Start()

    HandleVFX(GameState:game_state, PlayerID:string, Agent:agent)<suspends>:void=
        if (Guard := GuardPropertyID?). GameState.AfterTrue(ResourceID, Guard, PlayerID)

        if (V := VFX?):
            Position := RefPosition.GetTransform().Translation + Offset
            SpawnVFX := spawn_vfx_fn{ VFX := V, Position := Position }
            GameState.OnUpdated(ResourceID, PropertyID, PlayerID, SpawnVFX.FN)
