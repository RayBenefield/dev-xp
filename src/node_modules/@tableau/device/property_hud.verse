...
With<constructor>(Label:string) := with_label. Label := Label
with_label := class():
    Label<public>:string
...
property_hud<public> := class(creative_device, side_effect_device):
    ...
    GetSideEffects<override>(GameState:game_state, TypeID:string, PlayerID:string)<suspends>:[]side_effect=
        Widget := WhiteText()
        GameState.AddUI(Widget, PlayerID, HUDSettings)

        array:
            PushToMsg(Path(PropertyID), With(Label).FN, Widget.SetText)
            # Debug(Path(PropertyID), Label)
