...
CreateSelectorResource<public>(ResourceID:string, Items:[]string, Wrap:logic):resource=
    LabelProps:= CreateMap of for(Item:Items). ("{Item} Label", Str of Item)
    IsProps := CreateMap of for(Item:Items). ("Is {Item}", Bool of (Path("Selection"), "String Equals", Path("{Item} Label")))
    CreatedProps := ConcatenateMaps(LabelProps, IsProps)

    resource:
        ID := ResourceID
        Props := Combine(CreatedProps).WithAll of map:
...
selector_device<public> := class(creative_device, config_device):
    ...
    GetResources<override>():[]resource= array. CreateSelectorResource(ResourceID, BlockTypes, Wrap)