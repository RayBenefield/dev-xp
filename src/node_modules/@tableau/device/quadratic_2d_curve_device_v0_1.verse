...
quadratic_2d_curve_device_v0_1<public> := class(tableau_device):
    ...
    GetResources<override>():[]resource= array. resource:
        Props := map:
            ...
            "StartToControl" => Vec2 of (Path("Start"), "Lerp", Path("Control"), Path("Delta"))
            "ControlToEnd" => Vec2 of (Path("Control"), "Lerp", Path("End"), Path("Delta"))
            "Point" => Vec2 of (Path("StartToControl"), "Lerp", Path("ControlToEnd"), Path("Delta"))
            "X" => Float of (Path("Point"), "Vec2 X")
            "Y" => Float of (Path("Point"), "Vec2 Y")
