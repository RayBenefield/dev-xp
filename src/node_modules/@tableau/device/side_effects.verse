...
UpdateAnimWithBool<public><constructor>(Prop:?creative_prop, AnimCreator:anim_creator_bool) := update_anim_bool_fn:
    Prop := Prop
    AnimCreator := AnimCreator
    FrameOfReference := if (P := Prop?). ResetFrame(P.GetTransform()) else. Frame()
UpdateAnimWithFloat<public><constructor>(Prop:?creative_prop, AnimCreator:anim_creator_float) := update_anim_float_fn:
    Prop := Prop
    AnimCreator := AnimCreator
    FrameOfReference := if (P := Prop?). ResetFrame(P.GetTransform()) else. Frame()
update_anim_bool_fn<public> := class():
    ...
    FrameOfReference<public>:frame
    FN<public>(Value:logic):void= if (P := Prop?, A := P.GetAnimationController[]):
        Frames:= Concatenate(array. FrameOfReference, AnimCreator(Value))
        A.SetAnimation(Frames.Deltas(P.GetTransform()), ?Mode:=animation_mode.OneShot)
