...
number_device_v0_1<public> := class(tableau_device):
    ...
    GetResources<override>():[]resource= array. resource:
        Props := map:
            ...
            "Block Min" => Bool of (Path("Has Min"), "AND", Path("Is Min"))
            "Block Max" => Bool of (Path("Has Max"), "AND", Path("Is Max"))
            "Min Value" => Integer of (Path("Block Min"), "If True", Path("Min"))
            "Max Value" => Integer of (Path("Block Max"), "If True", Path("Max"))
        ...
        Actions := map:
            "Increment" => Action of array:
                IsFalse("Block Max").Else("Capped Reset")
                Set("Value").To("Next Value")
            "Decrement" => Action of array:
                IsFalse("Block Min").Else("Capped Reset")
