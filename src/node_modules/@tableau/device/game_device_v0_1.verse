...
game_device_v0_1 := class(creative_device):
    ...
    ProcessSideEffects(GameState:game_state, Device:tableau_device, ResourceID:string, SubTypeID:string)<suspends>:void=
        ...
        Debugs := for(Property:Device.GetDebugs(), Property <> ""). Debug(Property, ResourceID, SubID)
        SideEffects := Concatenate(NonDebugSideEffects, Debugs)

        for (Effect:SideEffects). if (E := debug_pipe[Effect]):
            GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)
            spawn. GameState.SyncStr(HardPath(ResourceID, Effect.PropertyID), SubID, E.FN)
        Sleep(0.0)
        ...
        for (Effect:SideEffects). if (E := first_pipe[Effect]). spawn. E.FN()
        Sleep(0.0)

        for (Effect:SideEffects, PP := HardPath(ResourceID, Effect.PropertyID)):
            if(Effect.PropertyID <> ""). GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)

            if (E := tap_pipe[Effect]). spawn. GameState.OnUpdated(PP, SubID, E.FN)
            if (E := bool_pipe[Effect]). spawn. GameState.SyncBool(PP, SubID, E.FN)
            if (E := int_pipe[Effect]). spawn. GameState.SyncInt(PP, SubID, E.FN)
            if (E := float_pipe[Effect]). spawn. GameState.SyncFloat(PP, SubID, E.FN)
            if (E := str_pipe[Effect]). spawn. GameState.SyncStr(PP, SubID, E.FN)
            if (E := agent_pipe[Effect]). spawn. GameState.SyncAgent(PP, SubID, E.FN)
            if (E := msg_pipe[Effect]). spawn. GameState.SyncStr(PP, SubID, E.FN)
            if (E := vec3_pipe[Effect]). spawn. GameState.SyncVec3(PP, SubID, E.FN)
            if (E := action_pipe[Effect]). spawn. GameState.AttachAction(PP, SubID, E.Event)
            if (E := arg_action_pipe[Effect]). spawn. GameState.AttachArgAction(PP, SubID, E.Event)
            if (E := sync_float_side_effect[Effect]). spawn. GameState.UpdateFloat(PP, SubID, E.From)
            if (E := widget_binding[Effect]). spawn. GameState.AddUI(E.Widget, SubID, E.Position)
