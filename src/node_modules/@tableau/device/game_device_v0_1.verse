...
game_device_v0_1 := class(creative_device):
    ...
    ProcessSideEffects(GameState:game_state, Device:tableau_device, ResourceID:string, SubTypeID:string)<suspends>:void=
        ...
        NonDebugSideEffects := Device.GetSideEffects(Scoped)
        Debugs := for(Property:Device.GetDebugs(), Property <> ""). Debug(Property)
        SideEffects := Concatenate(NonDebugSideEffects, Debugs)

        for (Effect:SideEffects). if (E := debug_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
        ...
        for (Effect:SideEffects):
            if (E := tap_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.OnUpdated(PropSource, SubTypeID, E.FN)
            if (E := bool_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncBool(PropSource, SubTypeID, E.FN)
            if (E := int_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncInt(PropSource, SubTypeID, E.FN)
            if (E := float_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncFloat(PropSource, SubTypeID, E.FN)
            if (E := str_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncStr(PropSource, SubTypeID, E.FN)
            if (E := agent_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncAgent(PropSource, SubTypeID, E.FN)
            if (E := msg_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncStr(PropSource, SubTypeID, E.FN)
            if (E := vec3_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.SyncVec3(PropSource, SubTypeID, E.FN)
            if (E := action_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.AttachAction(PropSource, SubTypeID, E.Event)
            if (E := trigger_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubTypeID)?, ActionSource := E.ActionSource.TryComplete(ResourceID, SubTypeID)?):
                spawn. GameState.TriggerAction(PropSource, SubTypeID, ActionSource, E.TestValue)
            if (E := sync_float_side_effect[Effect]):
                PropSource := HardPath(ResourceID, E.PropertyID)
                spawn. GameState.UpdateFloat(PropSource, SubTypeID, E.From)
