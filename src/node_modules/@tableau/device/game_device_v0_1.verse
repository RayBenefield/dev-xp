...
game_device_v0_1 := class(creative_device):
    ...
    ProcessSideEffects(GameState:game_state, Device:tableau_device, ResourceID:string, SubTypeID:string)<suspends>:void=
        ...
        Scoped := scoped_state{ GameState := GameState, Source := ResourceID, PlayerID := SubID }
        ...
        for (Effect:SideEffects):
            ...
            if (E := trigger_pipe[Effect], PropSource := E.PropertySource.TryComplete(ResourceID, SubID)?, ActionSource := E.ActionSource.TryComplete(ResourceID, SubID)?):
                spawn. GameState.TriggerAction(PropSource, SubID, ActionSource, E.TestValue)
