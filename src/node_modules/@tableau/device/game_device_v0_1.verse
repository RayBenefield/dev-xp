...
game_device_v0_1 := class(creative_device):
    ...
    ProcessSideEffects(GameState:game_state, Device:tableau_device, ResourceID:string, SubTypeID:string)<suspends>:void=
        ...
        for (Effect:SideEffects):
            ...
            if (E := str_pipe[Effect]):
                GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)
                spawn. GameState.SyncStr(PP, SubID, E.FN)
            if (E := agent_pipe[Effect]):
                GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)
                spawn. GameState.SyncAgent(PP, SubID, E.FN)
            if (E := msg_pipe[Effect]):
                GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)
                spawn. GameState.SyncStr(PP, SubID, E.FN)
            if (E := vec3_pipe[Effect]):
                GameState.PrimeCache(ResourceID, Effect.PropertyID, SubID)
                spawn. GameState.SyncVec3(PP, SubID, E.FN)
            if (E := action_pipe[Effect]):
                spawn. GameState.AttachAction(PP, SubID, E.Event)
            if (E := arg_action_pipe[Effect]):
                spawn. GameState.AttachArgAction(PP, SubID, E.Event)
