
using. /Fortnite.com/Devices
using. /Verse.org/Simulation

toggle_camera_device<public> := class(tableau_device):
    @editable PropertyID:string = ""

    @editable Angle:?gameplay_camera_fixed_angle_device = false
    @editable Point:?gameplay_camera_fixed_point_device = false
    @editable Orbit:?gameplay_camera_orbit_device = false

    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        Agent := State.GetSubAgent("Possessed By").Get()
        # AddCam := AddCamera(Agent, option{ Angle? or Point? or Orbit? })
        AddCam := if (Angle?). AddCamera(Agent, Angle) else if(Point?). AddCamera(Agent, Point) else. AddCamera(Agent, Orbit)
        array. PushBool(PropertyID, AddCam.FN)
