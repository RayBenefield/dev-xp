...
generator_test := class(creative_device, test_suite):
    ...
    GetTests<override>():[]test_case = array:
        test_case:
            Name := "Generator should not have an owner"
            Test := NoOwner
        test_case:
            Name := "Generator should have an owner"
            Test := Owner
        test_case:
            Name := "Generator Owner should not be maxed"
            Test := OwnerIsNotMaxed
        test_case:
            Name := "Generator Owner should be maxed"
            Test := OwnerIsMaxed
        test_case:
        ...
        test_case:
            Name := "Generator Owner should increase its count"
            Test := OwnerIncreaseCount
        test_case:
    ...
    NoOwner():?failure =
        ...
        # Act, Assert
        if (Generator.HasOwner[]). return fail("Generator shouldn't have an owner")
    ...
    Owner():?failure =
        ...
        # Act, Assert
        if (not Generator.HasOwner[]). return fail("Generator should have an owner")

        # Pass
        return false

    OwnerIsNotMaxed():?failure =
        # Arrange
        Generator := generator{ ID := 13, OwnerID := 26, MaxCount := 1, Counts := map{ 26 => 0 } }

        # Act, Assert
        if (Generator.IsMaxed[]). return fail("Generator shouldn't be maxed")

        # Pass
        return false

    OwnerIsMaxed():?failure =
        # Arrange
        Generator := generator{ ID := 13, OwnerID := 26, MaxCount := 1, Counts := map{ 26 => 1 } }

        # Act, Assert
        if (not Generator.IsMaxed[]). return fail("Generator should be maxed")
    ...
    IsNotMaxed():?failure =
        # Arrange
        Generator := generator{ ID := 13, OwnerID := 26, MaxCount := 1, Counts := map{ 26 => 1, 39 => 0 } }
    ...
    IsMaxed():?failure =
        # Arrange
        Generator := generator{ ID := 13, OwnerID := 26, MaxCount := 1, Counts := map{ 26 => 0, 39 => 1 } }
    ...
    OwnerIncreaseCount():?failure =
        # Arrange
        Generator := generator{ ID := 13, OwnerID := 26, MaxCount := 1, Counts := map{ 26 => 0 } }
        var NewGen:generator = Generator

        # Act
        if (Value := Generator.IncreaseCount[], set NewGen = Value):

        # Assert
        Count := NewGen.GetCount()
        if (Count = 0). return fail("Generator count didn't increase")
