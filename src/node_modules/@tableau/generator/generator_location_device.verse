...
generator_location_device<public> := class(creative_device, updatable):
    ...
    @editable Zone:vfx_spawner_device = vfx_spawner_device{}
    ...
    Update<override>()<suspends>:void=
        ...
        then:
            ...
            Owner.SetScoreAward(OwnerID)
            if (HasRequiredGenerators[Repo, InitialState, OwnerID]). Zone.Enable() else. Zone.Disable()
        ...
        then:
            ...
            Owner.SetScoreAward(OwnerID)
            if (HasRequiredGenerators[Repo, NewState, OwnerID]). Zone.Enable() else. Zone.Disable()
