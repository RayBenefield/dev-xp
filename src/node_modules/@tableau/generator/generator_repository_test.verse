...
generator_repository_test := class(creative_device, test_suite):
    ...
    GetTests<override>():[]test_case = array:
        ...
        test_case:
            Name := "Only Owned Generators with a Count should be fetchable by OwnerID"
            Test := FetchOwnedCountedGenerators
    ...
    FetchOwnedGenerators():?failure =
        ...
        ID := 13
        OwnerID := 26
        Generator := generator{ ID := ID, OwnerID := OwnerID }
        NotOwnedGenerator := generator{ ID := 12 }
        Repo := generator_repository. Initial := array{Generator, NotOwnedGenerator}

        # Act
        OwnedGenerators := Repo.GetOwnedGenerators(OwnerID)

        # Assert
        if (OwnedGenerators.Length <> 1). return fail("More than one generator was returned")
        if (OwnedGenerators[0].ID <> ID). return fail("First generator isn't the right one")

        # Pass
        return false

    FetchOwnedCountedGenerators():?failure =
        # Arrange
        OwnerID := 39
        NotCountedGenerator := generator{ ID := 13, OwnerID := OwnerID, Counts := map{ 39 => 0 } }
        CountedGenerator := generator{ ID := 26, OwnerID := OwnerID, Counts := map{ 39 => 1 } }
        NotOwnedGenerator := generator{ ID := 12 }
        Repo := generator_repository. Initial := array{NotCountedGenerator, CountedGenerator, NotOwnedGenerator}

        # Act
        OwnedGenerators := Repo.GetOwnedCountedGenerators(OwnerID)
