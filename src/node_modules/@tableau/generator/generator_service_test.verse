...
generator_service_test := class(creative_device, test_suite):
    ...
    GetTests<override>():[]test_case = array:
        ...
        test_case:
            Name := "Should buy a generator if requirements met, with no cost"
            Test := BuyWithRequiredAndNoCost
        test_case:
            Name := "Should buy a generator if requirements met, and can afford"
            Test := BuyWithRequiredAndCanAfford
        test_case:
            Name := "Should not buy a generator if requirements met, and can't afford"
            Test := NotBuyWithRequiredAndCannotAfford
        test_case:
            Name := "Should not buy a generator if requirements are not met, but can afford"
            Test := NotBuyWithoutRequiredAndCanAfford
        test_case:
            Name := "Should not buy a generator if requirements are not met, and can't afford"
    ...
    BuyWithRequiredAndCanAfford():?failure =
        ...
        Participant := participant{ ID := 26, Resources := map{ 1 => num. Value := 2.0 } }
        RequiredGenerator := generator{ ID := 13, Counts := map{ 26 => 1 } }
        Generator := generator{ ID := 14, Required := option. array. 13, Cost := Cost }
        Repo := generator_repository. Initial := array{ RequiredGenerator, Generator }
        ParticipantRepo := participant_repository. Initial := array. Participant
        Service := generator_service{ Repo := Repo, ParticipantRepo := ParticipantRepo, EffectProvider := effect_provider{} }
        Ev1 := Ev_GenPurchased(entity_generator{}, 14)
    ...
    NotBuyWithoutRequiredAndCanAfford():?failure =
        ...
        Cost := option. cost{ Amount := num. Value := 1.0, Type := 1 }
        Participant := participant{ ID := 26, Resources := map{ 1 => num. Value := 2.0 } }
        RequiredGenerator := generator{ ID := 13, Counts := map{ 26 => 0 } }
        Generator := generator{ ID := 14, Required := option. array. 13, Cost := Cost }
        Repo := generator_repository. Initial := array{ RequiredGenerator, Generator }
        ParticipantRepo := participant_repository. Initial := array. Participant
        Service := generator_service{ Repo := Repo, ParticipantRepo := ParticipantRepo, EffectProvider := effect_provider{} }
    ...
    NoRequirements():?failure =
        # Arrange
        # RequiredGenerator := generator{ ID := 13, Counts := map{ 26 => 0 } }
        Generator := generator{ ID := 14 }
        Repo := generator_repository. Initial := array{ Generator }

        # Act, Assert
        if(not HasRequiredGenerators[Repo, Generator, 26]). return fail("Should pass requirements")
    ...
    FailOneRequirement():?failure =
        ...
        # Act, Assert
        if(HasRequiredGenerators[Repo, Generator, 26]). return fail("Should not pass requirements")
    ...
    PassBothRequirements():?failure =
        # Arrange
        RequiredGenerator := generator{ ID := 13, Counts := map{ 26 => 1 } }
        RequiredGenerator2 := generator{ ID := 15, Counts := map{ 26 => 1 } }
        Generator := generator{ ID := 14, Required := option. array{ 13, 15 } }
        Repo := generator_repository. Initial := array{ RequiredGenerator, RequiredGenerator2, Generator }
    ...
    PassOneOfTwoRequirements():?failure =
        # Arrange
        RequiredGenerator := generator{ ID := 13, Counts := map{ 26 => 1 } }
        RequiredGenerator2 := generator{ ID := 15, Counts := map{ 26 => 0 } }
        Generator := generator{ ID := 14, Required := option. array{ 13, 15 } }
        Repo := generator_repository. Initial := array{ RequiredGenerator, RequiredGenerator2, Generator }
