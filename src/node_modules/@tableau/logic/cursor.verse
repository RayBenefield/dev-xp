...
GetAxis(C:coords):half_path = case(C):
    coords.X => Path("X")
    coords.XNeg => Path("X Neg")
    coords.Y => Path("Y")
    coords.YNeg => Path("Y Neg")
    coords.Neither => ZERO()
...
CreateCursorResource<public><constructor>(C:cursor_settings) := resource:
    ...
    Props := map:
        ...
        "+X" => prop_int{ Focus := Path("X"), FN := "+", Tweak := ONE() }
        "-X" => prop_int{ Focus := Path("X"), FN := "-", Tweak := ONE() }
        "+Y" => prop_int{ Focus := Path("Y"), FN := "+", Tweak := ONE() }
        "-Y" => prop_int{ Focus := Path("Y"), FN := "-", Tweak := ONE() }
        "Row Offset" => prop_int{ Focus := Path("Y"), FN := "-", Tweak := ONE() }
        ...
        "X Coord" => prop_float{ Focus := GetAxis(C.XAxis), FN := "*", Tweak := Path("Grid Size") }
        "Y Coord" => prop_float{ Focus := GetAxis(C.YAxis), FN := "*", Tweak := Path("Grid Size") }
        "Z Coord" => prop_float{ Focus := GetAxis(C.ZAxis), FN := "*", Tweak := Path("Grid Size") }
        ...
        "Partial ID" => prop_str{ Focus := Path("X"), FN := "Append", Tweak := COMMA() }
