...
CreateGridResource<public><constructor>(ID:string, C:grid_settings) := resource:
    ID := ID
    Props := map:
        "X" => (Integer of C.XSource?) or (Integer of C.X)
        "X Neg" => Integer of (Path("X"), "Negate")
        "Y" => (Integer of C.YSource?) or (Integer of C.Y)
