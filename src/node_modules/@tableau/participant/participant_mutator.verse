...
participant_mutator<public> := class(mutator):
    ...
    ApplyEvents<override>(Events:[]domain_event):void=
        for(Ev:Events):
            case(Ev.Event.Name()):
                "Participant Added" => spawn. AddParticipant(Ev.Subject)
                "Resource Added" => spawn. AddResource(Ev.Subject)
                "Participant Possessed" => spawn. PossessParticipant(Ev.Subject)
                # ev_resources_gained{}.Name() => {}
                "Resources Gained" => spawn. UpdateResource(Ev.Subject, Ev.Target)
                _ => {}

    AddParticipant(ParticipantID:string)<suspends>:void=
    ...
    PossessParticipant(ParticipantID:string)<suspends>:void=
        Participant := GameState.AwaitParticipant(ParticipantID)
        Participant.Agent.Set(AgentRepository.GetAgent(ParticipantID))

    UpdateResource(ParticipantID:string, ResourceID:string)<suspends>:void=
        Participant := GameState.AwaitParticipant(ParticipantID)
        if:
