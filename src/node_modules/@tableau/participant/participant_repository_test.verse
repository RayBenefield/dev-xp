...
participant_repository_test := class(creative_device, test_suite):
    ...
    Update():?failure =
        ...
        ID := "13"
        ResourceID := "26"
        Participant := participant{ ID := ID, Resources := map { ResourceID => num. Value := 1.23 } }
        Repo := participant_repository. Initial := array. Participant
        NewParticipant := participant{ ID := ID, Resources := map { ResourceID => num. Value := 2.46 } }

        # Act
        if (Part := Repo.Update[NewParticipant]):
            # Assert
            if (not Part.GetResource(ResourceID).Equals[num. Value := 2.46]). return fail("Resources was not updated")
        else:
