...
Execute<public><constructor>(Focus:half_path) := effect{ FN := "Execute Action", Focus := Focus }
execute_effect<public> := class():
    ...
    Affect<public>()<suspends>:?string=
        SwitchMap(Once(State.ToResourceID(Focus)), RunAction)
        return false

    RunAction(ResourceID:string)<suspends>:string_goo=
        PropertyID := Focus.PropertyID? or ""
        State.GetAction(ResourceID, PropertyID).Set()
        StrGoo()
