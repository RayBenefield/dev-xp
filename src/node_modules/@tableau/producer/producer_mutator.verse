...
producer_mutator<public> := class(mutator):
    ...
    BuyEffect<public>:effect

    ApplyEvents<override>(Events:[]domain_event):void=
        for(Ev:Events):
            case(Ev.Event.Name()):
                "Producer Added" => spawn. AddProducer(Ev.EntityID)
                "Producer Purchased" => spawn. PurchaseProducer(Ev.EntityID, Ev.SubEntityID)
                _ => {}

    AddProducer(ID:int)<suspends>:void=
        if (Initial := Repo.GetByID[ID]):
            Producer := GameState.AddProducer(Initial, BuyEffect)
            Participants := ParticipantRepository.GetAll()
            for (Participant:Participants):
                Producer.AddParticipant(Participant.ID, Initial)

    PurchaseProducer(ID:int, ParticipantID:int)<suspends>:void=
