...
buy_producer_ui<public> := class():
    ...
    OnPressed(Msg:widget_message):void = BuyEvent.Signal(VM.ID)
...
producer_pos_ui<public> := class():
    ...
    BuyEvent<public>:event(int) = event(int){}
    AddForPlayer<public>(Player:player)<suspends>:void=
        ...
        Events := for(UI:ProducerUIs). UI.BuyEvent
        loop:
            MapRace(Events, MulticastBuy)

    MulticastBuy(Event:event(int))<suspends>:void=
        ID := Event.Await()
        BuyEvent.Signal(ID)
