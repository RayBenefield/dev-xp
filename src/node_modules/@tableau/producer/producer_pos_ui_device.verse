...
producer_pos_ui_device<public> := class(creative_device, updatable):
    ...
    Command_Buy(UI:producer_pos_ui)<suspends>:void=
        ID := UI.BuyEvent.Await()
        if (Effect := BuyEffect?):
            Effect.Play(
                command:
                    Subjects := array. ID
