...
producer_pos_ui_device<public> := class(creative_device, updatable):
    ...
    @editable ProducerIDs<public>:[]string = array{}
    ...
    var BuyEffector:?oldeffector = false
    var ProduceEffector:?oldeffector = false

    # Local State Management
    var States:[string]producer_state = map{}
    Bought:event(string) = event(string){}
    Produce:event(string) = event(string){}
    Clear:event() = event(){}
        ...
        for (State:States, VM := State.VM, ID := VM.ID, Count := State.Count):
            ...
            then:
                if (Cost := Prod.GetCost[Part.ID]):
                    VM.Cost.Set("Cost: {Cost.Amount}")
                VM.Count.Set("Count: {Prod.GetCount(Part.ID)}")
    ...
    UpdateVFX(State:producer_vfx_state, ID:string, Current:int, New:int)<suspends>:void=
        case(State):
    ...
    Initialize<public>(
        ...
        if (
            ...
            Widget := ProducerPosUI.Initialize()
            Canvas := canvas. Slots := array. canvas_slot. Widget := Widget
        ...
        for(State:States, ID := State.VM.ID):
            if (Prod := ProducerRepo.GetByID[ID]):
                State.VM.Name.Set("Buy x1 {Prod.Name}")
    ...
    ProduceTimer(ID:string)<suspends>:void=
        Bought.Await()
        loop:
