...
producer_pos_hud_device<public> := class(creative_device, tableau_device):
    ...
    ProdMsg<localizes>(Amount:string):message := "Production: {Amount}"
    ...
    SetupHUD(GameState:game_state, ParticipantID:string):game_hud=
        ...
        do:
            ...
            ProdVM := Map(Map(Production.ValueFor(ParticipantID), ToStr), ProdMsg)
            VM := producer_pos_vm:
                ID := ProducerID
                Name := gooey_message. VM := Map(Name, BuyMsg)
                Cost := gooey_message. VM := CostVM
                Count := gooey_message. VM := CountVM
                Production := gooey_message. VM := ProdVM
