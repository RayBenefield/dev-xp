...
producer_service_test := class(creative_device, test_suite):
    ...
    NoRequirements():?failure =
        ...
        # Act, Assert
        if(not HasRequiredProducers[Repo, Producer, 26]). return fail("Should pass requirements")
    ...
    FailOneRequirement():?failure =
        ...
        # Act, Assert
        if(HasRequiredProducers[Repo, Producer, 26]). return fail("Should not pass requirements")
    ...
    VFXState_Maxed():?failure =
        ...
        # Act
        Result := WhichVFXState(Repo, Producer, Participant)
