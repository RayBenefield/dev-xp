...
producer_ability<public> := class<concrete>:
    @editable Subject:producer_string_lenses = producer_string_lenses.Target
    @editable Target:producer_string_lenses = producer_string_lenses.ResourceID
    @editable Pay:producer_num_lenses = producer_num_lenses.ProductionAmount
...
producer<public> := class<concrete>():
    ...
    @editable Required<public>:?[]string = false
    @editable DefaultRate<public>:float = 1.0

    # [ParticipantID] => 0
    Counts<public>:[string]int = map{}
    # [ParticipantID] => [DefaultRate]
    Rates<public>:[string]float = map{}
    ...
    GetCost<public>(ParticipantID:string)<decides><transacts>:cost =
        BaseCost := HasCost[]
        if (FirstIsFree? and GetCount(ParticipantID) = 0):
            ...
            cost:
                ...
                Type := BaseCost.Type
    GetCount<public>(ParticipantID:string)<computes>:int = Counts[ParticipantID] or 0
    GetRate<public>(ParticipantID:string)<computes>:float = Rates[ParticipantID] or DefaultRate
    IsMaxed<public>(ParticipantID:string)<decides><transacts>:void = GetCount(ParticipantID) >= MaxCount

    GetProductionAmount<public>(ParticipantID:string):num=
        Count := GetCount(ParticipantID)
        if (Count = 0). return num{}
        Production * Count

    IncreaseCount<public>(ParticipantID:string)<decides><transacts>:producer=
        if(IsMaxed[ParticipantID]). false?

        var NewCounts:[string]int = Self.Counts
    ...
    OwnedBy<public>(ParticipantID:string)<decides><transacts>:int=
