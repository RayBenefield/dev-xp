...
producer_service<public> := class(service):
    ...
    Lens_ProducerProductionAmount(Prod:producer):[]num = array. Prod.Production
    ...
    ProducerAbilityToCommand<public>(Ability:producer_ability, Prod:producer):command=
        ...
        NumLenses:[producer_num_lenses]prod_num_lens = map:
            producer_num_lenses.ProductionAmount => Lens_ProducerProductionAmount
...
WhichVFXState<public>(All:[int]producer, Prod:producer, Part:participant)<transacts>:producer_vfx_state=
    ...
    if (not Prod.Cost? and HasRequiredProducers[All, Prod, Part.ID]). return producer_vfx_state.AvailableAndFree
    if (Cost := Prod.Cost?):
