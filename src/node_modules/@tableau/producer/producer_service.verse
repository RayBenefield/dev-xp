...
prod_string_lens := type{_(:producer):string}
...
producer_service<public> := class(service):
    ...
    Buy<public>(ProducerID:string, BuyerID:string):[]domain_event=
            ...
            if (NewProducer := Prod.IncreaseCount[BuyerID]):
                if (Repo.Update[NewProducer]):
                    set DomainEvents += array:
                        Ev_ProducerPurchased(Prod.ID, BuyerID)
    ...
    # EXPERIMENTAL
    #
    Produce<public>(ProducerID:string, ParticipantID:string):[]domain_event=
        var DomainEvents:[]domain_event = array{}
        ...
        then:
            for(Ability:Abilities):
                ...
                # for (Effector:LastCommandModification.AfterEffectors):
                #     set LastCommandModification = Effector.Play(LastCommandModification, payload{})

        return DomainEvents


    Lens_ProducerID(Prod:producer):string = Prod.ID
    Lens_ProducerResourceID(Prod:producer):string = if (ResID := Prod.ResourceID?). ResID else. ""
    # TODO: Add as producer domain object function
    Lens_ProducerAllOwners(Prod:producer):string = ""

    ProducerAbilityToCommand<public>(Ability:producer_ability, Prod:producer, ParticipantID:string):command=
        IntLenses:[producer_string_lenses]prod_string_lens = map:
            producer_string_lenses.ID => Lens_ProducerID
            producer_string_lenses.ResourceID => Lens_ProducerResourceID
            producer_string_lenses.AllOwners => Lens_ProducerAllOwners
            producer_string_lenses.Target => lens_ProducerTarget{ Target := ParticipantID }.Lens
        ...
        return if:
            SubjectLens := IntLenses[Ability.Subject]
            TargetLens := IntLenses[Ability.Target]
            PayloadLens := NumLenses[Ability.Payload]
        then:
            command:
                Subject := SubjectLens(Prod)
                Target := TargetLens(Prod)
                Payload := payload. Num := PayloadLens(Prod)
        else:
            command{}

lens_ProducerTarget := class():
    Target:string
    Lens(Prod:producer):string = Target

lens_ProductionAmount := class():
    Target:string
    Lens(Prod:producer):num = Prod.GetProductionAmount(Target)


HasRequiredProducers<public>(All:[string]producer, Prod:producer, ParticipantID:string)<decides><transacts>:void=
...
ToString<public>(State:producer_vfx_state):string=
    case(State):
        ...
        producer_vfx_state.Bought => "Bought"
        producer_vfx_state.Maxed => "Maxed"

WhichVFXState<public>(All:[string]producer, Prod:producer, Part:participant, OldCount:int)<transacts>:producer_vfx_state=
    Count := Prod.GetCount(Part.ID)
    if (Count > OldCount). return producer_vfx_state.Bought
