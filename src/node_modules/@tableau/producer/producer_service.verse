...
WhichVFXState<public>(Repo:producer_repository, Prod:producer, Part:participant)<transacts>:producer_vfx_state=
    if (Prod.IsMaxed[Part.ID]). return producer_vfx_state.Maxed
    if (not HasRequiredProducers[Repo, Prod, Part.ID]). return producer_vfx_state.NotAvailable
    if (not Prod.Cost? and HasRequiredProducers[Repo, Prod, Part.ID]). return producer_vfx_state.AvailableAndFree
    if (Cost := Prod.Cost?):
...
CanAfford<public>(ParticipantID:participant, Cost:cost)<decides><transacts>:void=
    Amount := ParticipantID.GetResource(Cost.Type)
