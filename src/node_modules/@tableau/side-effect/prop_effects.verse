...
move_prop<public> := class():
    ...
    FN<public>(Value:vector3)<suspends>:void=
        if (Time <> 0.0). OneShot(Prop, Frame(MoveTo(Value + Offset)).SetTime(Time).SetEasing(EaseIn))
        else. TeleportTo(Prop, Value + Offset)
...
update_anim_float_fn<public> := class():
    ...
    FN<public>(Value:float):void= SetupOneShot(Prop, AnimCreator(Value))
...
ToggleAnim<public><constructor>(Prop:?creative_prop)<transacts> := toggle_anim_fn. Prop := Prop
toggle_anim_fn<public> := class():
    Prop<public>:?creative_prop = false
    FN<public>(Value:logic):void= if (P := Prop?, A := P.GetAnimationController[]):
        if (Value?). A.Play() else. A.Stop()
...
SetMaterial<public><constructor>(Prop:?creative_prop, MaterialMap:[string]material)<transacts> := set_material_fn{ Prop := Prop, MaterialMap := MaterialMap }
SetMaterial<public><constructor>(Prop:?creative_prop, MaterialMap:[string]material, Index:int)<transacts> := set_material_fn{ Prop := Prop, MaterialMap := MaterialMap, Index := Index }
set_material_fn<public> := class():
    Prop<public>:?creative_prop = false
    MaterialMap<public>:[string]material
    Index<public>:int = 0
    FN<public>(Value:string)<suspends>:void= if (P := Prop?, Mat := MaterialMap[Value]). P.SetMaterial(Mat, ?Index := Index)
