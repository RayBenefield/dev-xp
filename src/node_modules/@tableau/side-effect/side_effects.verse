...
msg_pipe<public> := class(str_side_effect):
    PropertyID<override>:string = ""
    To<public>:msg_tap
    FN<override>(Value:string)<suspends>:void= To(Str2Msg(Value))

Debug<public><constructor>(PropertyID:string)<transacts> := debug_pipe{ PropertyID := PropertyID }
Debug<public><constructor>(PropertyID:string, ResourceID:string, SubID:string)<transacts> := debug_pipe{ PropertyID := PropertyID, ResourceID := option. ResourceID, SubID := option. SubID }
debug_pipe<public> := class(side_effect):
    ...
    FN<public>(Value:string)<suspends>:void=
        FinalLabel := "[{Label?}]" or "{ResourceID?}.{PropertyID}@{SubID?}" or "[{PropertyID}]"
