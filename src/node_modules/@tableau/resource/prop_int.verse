...
StrToInt<public>(Value:string)<transacts>:int= if(Value <> ""). 1 else. 0
...
IntSquare(First:int)<computes>:int= First * First
...
IfZeroInt(Value:int, IfZero:int)<computes>:int= if(Value = 0). IfZero else. Value
...
int_plug<public> := class<concrete>():
    @editable Default:int = 1
    @editable Source:?prop_path = false
Integer<public>(Plug:int_plug)<transacts>:prop_int := (Integer of Plug.Source?) or (Integer of Plug.Default)
int_var<public> := class<concrete>(prop_var):
    @editable Default:int = 0
Integer<public>(Plug:int_var)<transacts>:prop_int= if(Plug.Source.Empty[]). Integer(Plug.Default) else. Integer(Plug.Source)
...
Integer<public><constructor>(Focus:prop_path, FN:string, Tweak:prop_path)<transacts> := prop_int{ Focus := Focus.ToPath(), FN := FN, Tweak := Tweak.ToPath() }
...
prop_int<public> := class<concrete>(prop):
    ...
    ValueFor<public>(State:scoped_state):int_goo= case(FN):
        ...
        "Square" => IntGoo(Map(State.ToInt(Focus), IntSquare))
        ...
        "If Zero" => IntGoo(Computed(State.ToInt(Focus), State.ToInt(Tweak), IfZeroInt))
        ...
        _ => case(Type):
            ...
            prop_type.StrType => IntGoo(Map(State.ToStr(Focus), StrToInt))
