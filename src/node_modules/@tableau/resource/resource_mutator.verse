    ...
    UpdateProperty(Ev:domain_event)<suspends>:void=
        ...
        then:
            case(PropType):
                prop_type.StatNum =>
                    if:
                        Property := Resource.StatProps.Nums[PropertyID]
                        Value := Res.StatNums[PropertyID]
                    then:
                        Property.Set(Value)
                prop_type.StatFloat =>
                    if:
                        Property := Resource.StatProps.Floats[PropertyID]
                        Value := Res.StatFloats[PropertyID]
                    then:
                        Property.Set(Value)
                prop_type.StatStr =>
                    if:
                        Property := Resource.StatProps.Strs[PropertyID]
                        Value := Res.StatStrs[PropertyID]
                    then:
                        Property.Set(Value)
                prop_type.StatBool =>
                    if:
                        Property := Resource.StatProps.Bools[PropertyID]
                        Value := Res.StatBools[PropertyID]
                    then:
                        Property.Set(Value)
                prop_type.Num =>
                    if:
