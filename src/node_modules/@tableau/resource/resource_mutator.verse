...
resource_mutator<public> := class(mutator):
    ...
    ApplyEvents<override>(Events:[]domain_event):void=
        for(Ev:Events):
            Handler := case(Ev.Event.Name()):
                ...
                "Resource Renamed" => spawn. RenameResource(Ev.Subject)
    ...
    RenameResource(ResourceID:string)<suspends>:void=
    ...
    UpdateProperty(Ev:domain_event)<suspends>:void=
        ...
        if:
            ...
            PropValues := Res.PropertyValues[PropertyID]
            Value := PropValues[ParticipantID]
