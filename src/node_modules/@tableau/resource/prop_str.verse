...
IfBlankStr(Value:string, IfBlank:string)<computes>:string= if (Value = ""). IfBlank else. Value

str_plug<public> := class<concrete>():
    @editable Default:string = ""
    @editable Source:?prop_path = false
Str<public>(Plug:str_plug)<transacts>:prop_str := (Str of Plug.Source?) or (Str of Plug.Default)
...
Str<public><constructor>(Focus:prop_path)<transacts> := prop_str. Focus := Focus.ToPath()
...
Str<public><constructor>(Focus:prop_path, FN:string, Tweak:prop_path)<transacts> := prop_str{ Focus := Focus.ToPath(), FN := FN, Tweak := Tweak.ToPath() }
MaybeStr<public><constructor>(Focus:?prop_path)<transacts> := prop_str. Focus := Focus?.ToPath() or EmptyPath()
...
prop_str<public> := class<concrete>(prop):
    ...
    ValueFor<public>(State:scoped_state):str_goo= case(FN):
        ...
        "If Blank" => StrGoo(Computed(State.ToStr(Focus), State.ToStr(Tweak), IfBlankStr))
        _ => case(Type):
