...
Vec3ToVec2(Value:vector3):vector2= V2(Value.X, Value.Y)
Vec2Lerp(First:vector2, Second:vector2, Delta:float)<transacts>:vector2= vector2:
    X := Lerp(First.X, Second.X, Delta)
    Y := Lerp(First.Y, Second.Y, Delta)
AppendFloats(X:float, Y:float)<transacts>:vector2= vector2{ X := X, Y := Y }
OnlyXs(First:vector2, Second:vector2)<transacts>:vector2= vector2{ X := First.X, Y := Second.X }
OnlyYs(First:vector2, Second:vector2)<transacts>:vector2= vector2{ X := First.Y, Y := Second.Y }
...
Vec2<public><constructor>(Focus:half_path, FN:string, Tweak:half_path, Braid:half_path)<transacts> := prop_vec2{ Focus := Focus, FN := FN, Tweak := Tweak, Braid := Braid }
...
Vec2<public><constructor>(Focus:prop_path, FN:string, Tweak:prop_path, Braid:prop_path)<transacts> := prop_vec2{ Focus := Focus.ToPath(), FN := FN, Tweak := Tweak.ToPath(), Braid := Braid.ToPath() }
...
prop_vec2<public> := class<concrete>(prop):
    ...
    ValueFor<public>(State:scoped_state):vec2_goo= case(FN):
        ...
        "Only Xs" => Vec2Goo(Computed(State.ToVec2(Focus), State.ToVec2(Tweak), OnlyXs))
        "Only Ys" => Vec2Goo(Computed(State.ToVec2(Focus), State.ToVec2(Tweak), OnlyYs))
        "Lerp" => Vec2Goo(Entwine(State.ToVec2(Focus), State.ToVec2(Tweak), State.ToFloat(Braid), Vec2Lerp))
        _ => case(Type):
            prop_type.Vec3Type => Vec2Goo(Map(State.ToVec3(Focus), Vec3ToVec2))
