using { /Verse.org/Simulation }

using { Tableau }

CopyResource<constructor>(Old:resource)<transacts> := resource:
    ID := Old.ID
    Name := Old.Name
    StaticProperties := Old.StaticProperties
    Properties := Old.Properties
    Abilities:= Old.Abilities
    PropertyValues := Old.PropertyValues

property<public> := class<concrete>():
    @editable ID<public>:string = ""
    @editable Default<public>:num = num{}

ability<public> := class<concrete>():
    @editable ID<public>:string = ""
    @editable Effects<public>:[]effect = array{}

resource<public> := class<unique><concrete>():
    @editable ID<public>:int = 0
    @editable Name<public>:string = ""
    @editable StaticProperties<public>:[]property = array{}
    @editable Properties<public>:[]property = array{}
    @editable Abilities<public>:[]ability = array{}
    var PropertyValues<public>:[string][int]num = map{}

    SetProperty<public>(PropertyID:string, ParticipantID:int, Value:num)<decides><transacts>:resource=
        var NewValues:[string][int]num = Self.PropertyValues
        if (not NewValues[PropertyID]):
            var NewProperty:[int]num = map{}
            if:
                set NewProperty[ParticipantID] = Value
                set NewValues[PropertyID] = NewProperty
        else:
            if (set NewValues[PropertyID][ParticipantID] = Value) {}

        return resource:
            CopyResource<constructor>(Self)
            PropertyValues := NewValues