...
resource<public> := class<unique><concrete>():
    ...
    var PropInts<public>:[string][string]int = map{}
    ...
    SetIntProperty<public>(PropertyID:string, PlayerID:string, Value:int)<decides><transacts>:resource=
        var NewValues:[string][string]int = Self.PropInts
        if (not NewValues[PropertyID]):
            var NewProperty:[string]int = map{}
            if:
