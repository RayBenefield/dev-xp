...
PlugCheck<public>(Plug:?t where t:type)<transacts>:prop_bool := Bool of (if(Plug?). true else. false)
...
Bool<public><constructor>(Focus:prop_path, FN:string, Tweak:prop_path)<transacts> := prop_bool{ Focus := Focus.ToPath(), FN := FN, Tweak := Tweak.ToPath() }
