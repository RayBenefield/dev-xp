...
resource_vm<public> := class():
    ...
    ID<public>:string
    var StatProps<public>:[string]gooey_string = map{}
    var Props<public>:[string]resource_property_vm = map{}
    var Abilities<public>:[string]action_maker = map{}

    Init<public>(Resource:resource)<suspends>:void =
        for (Property:Resource.StatProps):
            VM := Property.ToVM()
            if:
                set StatProps[Property.ID] = VM
        for (Property:Resource.Props):
            VM := resource_property_vm{ Default := Property.Default }
            spawn. VM.Init(Property.ID)
            if:
                set Props[Property.ID] = VM
