    ...
    ValueFor<public>(ParticipantID:string):gooey_bool =
        ...
        Result := case(Computer.FN):
            ">=" =>
                ...
                Pipe := GameState.GetNumProp(ResourceID, Computer.Pay.Str, ParticipantID)
...
GTE(First:num, Second:num):logic = if (First.GreaterThanOrEqual[Second]). true else. false
...
action<public> := class():
    ...
    Run(Pay:load)<suspends>:void=
        ...
        loop:
            if:
                Effect := Effects[Iterator]
                Effector := GameState.EffectorProvider.Get[Effect.FN]
            then:
                Command := Effector.Play(LastCommandModification, Effect.Pay)

                if (Next := Command?):
                    set LastCommandModification = Next
                else:
                    break
