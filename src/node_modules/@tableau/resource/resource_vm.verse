...
resource_comp_num_property_vm<public> := class():
    ...
    ValueFor<public>(ParticipantID:string):gooey_num =
        ...
        for:
            Data:Transformers
            # Transformer:num_transformer = GameState.NumTransformerProvider.Get[Data.Type]
            Payload := Data.Payload
        do:
            case(Data.Type):
                "Multiply" =>
                    Pipe := GameState.GetNumProp(ResourceID, Payload.Str, ParticipantID)
                    set VM = gooey_num. VM := Computed(VM, Pipe, NumMultiply)
                "Power" =>
                    Pipe:gooey_float = GameState.GetFloatProp(ResourceID, Payload.Str, ParticipantID)
                    set VM = gooey_num. VM := Computed(VM, Pipe, NumPower)
                "Reverse Power" =>
                    Pipe:gooey_num = GameState.GetNumProp(ResourceID, Payload.Str, ParticipantID)
