...
resource_comp_num_property_vm<public> := class():
    ...
    ValueFor<public>(PlayerID:string):gooey_num =
        # Check Cache
        ...
        Result := case(Computer.FN):
            "Multiply" =>
                NewVM := gooey_num{}
                Base:gooey_num = GameState.GetNumProp(ResourceID, BaseProp, PlayerID)
            ...
            "Reverse Power" =>
                NewVM := gooey_num{}
                Base:gooey_float = GameState.GetFloatProp(ResourceID, BaseProp, PlayerID)
...
resource_comp_bool_property_vm<public> := class():
    ...
    # TODO: Switch to NumTransformProvider when parametrics support type aliasing
    ValueFor<public>(PlayerID:string):gooey_bool =
        # Check Cache
        if (Value := Values[PlayerID]). return Value
...
resource_float_property_vm<public> := class():
    ...
    # TODO: Make transacts when I can get rid of the `Init`
    ValueFor<public>(PlayerID:string):gooey_float =
        if (Value := Values[PlayerID]). return Value
        VM := gooey_float{}
        spawn. VM.Init(Default)
        if. set Values[PlayerID] = VM
...
resource_str_property_vm<public> := class():
    ...
    # TODO: Make transacts when I can get rid of the `Init`
    ValueFor<public>(PlayerID:string):gooey_string =
        if (Value := Values[PlayerID]). return Value
        VM := gooey_string{}
        spawn. VM.Init(Default)
        if. set Values[PlayerID] = VM
...
resource_agent_property_vm<public> := class():
    ...
    # TODO: Make transacts when I can get rid of the `Init`
    ValueFor<public>(PlayerID:string):gooey_agent =
        if (Value := Values[PlayerID]). return Value
        VM := gooey_agent{}
        spawn. VM.Init(Default)
        if. set Values[PlayerID] = VM
