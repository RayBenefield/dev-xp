...
comp_props_vm<public> := class():
    ...
    var Agents<public>:[string]resource_comp_agent_property_vm = map{}
    ...
    Init<public>(Resource:resource)<suspends>:void =
        ...
        for (PropID->Prop:Resource.Props.Computed):
            case(Prop.GetType()):
                ...
                prop_type.Agent =>
                    if (Property := comp_agent[Prop]):
                        VM := resource_comp_agent_property_vm:
                            GameState := GameState
                            ResourceID := Resource.ID
                            Focus := Property.Focus
                            FN := Property.FN
                            Pay := Property.Pay
                        if. set CompProps.Agents[PropID] = VM
                _ => {}
...
resource_comp_agent_property_vm<public> := class():
    GameState<public>:game_state
    ResourceID<public>:string
    Focus<public>:half_path
    FN<public>:string
        ...
        loop:
            if (Effect := Effects[Iterator]):
                if (Effector := GameState.EffectorProvider.Get[Effect.FN]):
                    ...
                    OverridenPay := Pay.Override(Effect.Pay)
                    EffectPay := if (P := Effect.Pay.HalfPath?):
                        ResolvedProp := GameState.ResolvePath(Subject, P, Target)
                        WithProp(OverridenPay, ResolvedProp)
                    else:
                        OverridenPay
