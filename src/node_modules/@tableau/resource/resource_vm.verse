...
resource_vm<public> := class():
    ...
    StoreProp(PropID:string, Prop:prop):void=
        ScopedState := resource_state{ GameState := GameState, ResourceID := ResourceID }
        if. set PropTypes[PropID] = Prop.Type
        case(Prop.Type):
            prop_type.Integer => if (P := prop_int[Prop]):
                if. set Props.Ints[PropID] = int_vm{ ScopedState := ScopedState, Prop := P }
            prop_type.Num => if (P := prop_num[Prop]):
                if. set Props.Nums[PropID] = num_vm{ ScopedState := ScopedState, Prop := P }
            prop_type.Float => if (P := prop_float[Prop]):
                if. set Props.Floats[PropID] = float_vm{ ScopedState := ScopedState, Prop := P }
            ...
            prop_type.Str => if (P := prop_str[Prop]):
                if. set Props.Strs[PropID] = str_vm{ ScopedState := ScopedState, Prop := P }
            prop_type.Vec2 => if (P := prop_vec2[Prop]):
                if. set Props.Vec2s[PropID] = vec2_vm{ ScopedState := ScopedState, Prop := P }
            prop_type.Vec3 => if (P := prop_vec3[Prop]):
                if. set Props.Vec3s[PropID] = vec3_vm{ ScopedState := ScopedState, Prop := P }
            prop_type.Agent => if (P := prop_agent[Prop]):
                if. set Props.Agents[PropID] = agent_vm{ ScopedState := ScopedState, Prop := P }
            _ => {}
    ...
    GetStrProp<public>(PropID:string, PlayerID:string)<suspends>:string_goo=
        ...
        case(Type):
            prop_type.Integer => if (P := Props.Ints[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Num => if (P := Props.Nums[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Float => if (P := Props.Floats[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Bool => if (P := Props.Bools[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Str => if (P := Props.Strs[PropID]). P.ValueFor(PlayerID) else. StrGoo("")
            prop_type.Vec2 => if (P := Props.Vec2s[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Vec3 => if (P := Props.Vec3s[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            prop_type.Agent => if (P := Props.Agents[PropID]). P.AsStr(PlayerID) else. StrGoo("")
            _ => StrGoo("")
