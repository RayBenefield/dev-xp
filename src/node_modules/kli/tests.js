...
describe('KLI', it => {
    ...
    it('should error when commands is not an array', assert =>
        assert.throws(() => kli({ commands: {} }), 'Commands must be an array'))
    ...
    it('should error if any command does not have a name', assert =>
        assert.throws(
            () =>
                kli({
                    commands: [
    ...
    it('should run default command when nothing is given', assert => {
        const testCommand = {
            name: 'test',
            run: () => assert.pass(),
        }
    ...
    it('should run default command when set as a name', assert => {
        const cli = kli({
            default: 'test',
            commands: [
                {
    ...
    it('should run default command when empty array is given', assert => {
        const testCommand = {
            name: 'test',
            run: () => assert.pass(),
        }
    ...
    it('should not run effect when no commit is passed', assert => {
        const cli = kli({
            commands: [
                {
                    name: 'run',
    ...
    it('should run effect when commit is passed', assert => {
        const cli = kli({
            commands: [
                {
                    name: 'run',
    ...
    it('should run effect when command is set to autoCommit', assert => {
        const cli = kli({
            commands: [
                {
                    name: 'run',
    ...
    it('should not run effect when command is autoCommit, but dry run is given', assert => {
        const cli = kli({
            commands: [
                {
                    name: 'run',
