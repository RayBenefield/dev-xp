...
describe('Take Operator', function()
    describe('take()', function()
        ...
        it('is included in rx object', function()
            assert.no.is_nil(_.take)
        end)

        it('takes nothing from 1 is bypassed', function()
            local test_ = rx.of('hello').pipe(_.take())
            --
            expect(test_).to.produce('hello')
        end)

        it('takes nothing from 2 is bypassed', function()
            local test_ = rx.from({'hello', 'world'}).pipe(_.take())
            --
            expect(test_).to.produce('hello', 'world')
        end)

        it('takes 0 from 1 is emptied', function()
            local test_ = rx.of('world').pipe(_.take(0))
            --
            expect(test_).to.produce()
        end)

        it('takes 0 from 1 is emptied', function()
            local test_ = rx.from({'world', 'folks'}).pipe(_.take(0))
            --
            expect(test_).to.produce()
        end)

        it('takes 1 from 1', function()
            local test_ = rx.of('world').pipe(_.take(1))
            --
            expect(test_).to.produce('world')
        end)

        it('takes 1 from 3', function()
            local test_ = rx.from({'world', 'folks', 'people'}).pipe(_.take(1))
            --
            expect(test_).to.produce('world')
        end)

        it('takes 2 from 3', function()
            local test_ = rx.from({'world', 'folks', 'people'}).pipe(_.take(2))
            --
            expect(test_).to.produce('world', 'folks')
        end)

        it('finish even if threshold is not reached', function()
            local test_ = rx.from({'world', 'folks', 'people'}).pipe(_.take(5))
            --
