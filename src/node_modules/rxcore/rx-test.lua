...
local produce = function(state, args, level)
    ...
    local expectedNext = args
    expectedNext.n = nil

    local onNext, onError, onComplete = observableSpy(_)
    local nextCalls = onNext.calls

    local assertCalls = #args == #nextCalls
    if (not assertCalls) then util.printTable(expectedNext, 'Expected') end
    assert(assertCalls, 'expected '..#args..' calls, received '..#nextCalls..' calls')
