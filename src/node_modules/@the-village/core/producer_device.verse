...
producer_device<public> := class(tableau_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        BuildingLoc := Building?.GetTransform().Translation or V3()
        ...
        array:
            ...
            PushStr("Has Some", SetMaterial(Building, HasSomeMaterials).FN)
            Push("Count", SpawnVFX(BuildVFX, BuildingLoc).FN)
            Push("Produced Count", SpawnVFX(ProduceVFX, VFXLoc).FN)
