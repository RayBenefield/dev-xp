...
producer_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    GetSideEffects<override>(GameState:game_state, ProducerID:string, PlayerID:string)<suspends>:[]side_effect=
        ...
        Duration := GameState.GetFloatProp(ProducerID, "Production Rate", PlayerID).Get()
        if (Anim := Bar?.GetAnimationController[]):
            Anim.SetAnimation(array. Frame(ScaleXBy(-1.0)).Delta(), ?Mode:=animation_mode.OneShot)
            Anim.Play()
            Sleep(0.0)
            Anim.SetAnimation(BarAnim(Duration).Deltas(), ?Mode:=animation_mode.OneShot)

        Tier := GameState.GetIntProp(ProducerID, "Tier", PlayerID).Get()
        ProduceVFX := ProduceVFXs[Tier-1] or TycoonPack.Particles.Money.P_CoinBurst_1

        if (Anim := Building.GetAnimationController[]):
            Anim.SetAnimation(BuildAnim().Deltas(), ?Mode:=animation_mode.OneShot)

        array:
            play_animation{ PropertyID := "Start Produced Count", Prop := Bar }
            toggle_prop{ PropertyID := "Exists", Prop := option. Building }
            toggle_material{ PropertyID := "Has Some", Prop := option. Building, True := Summer, False := Hologram }
            spawn_vfx{ PropertyID := "Count", Location := BuildingLoc, VFX := BuildVFX }
            spawn_vfx{ PropertyID := "Produced Count", Location := VFXLoc, VFX := option. ProduceVFX }
            play_animation{ PropertyID := "Has Some", Prop := option. Building }
