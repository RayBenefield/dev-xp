...
producer_progress_device<public> := class(creative_device, tableau_device):
    ...
    Init<override>(GameState:game_state)<suspends>:void=
        Watcher := player_watcher{ GameState := GameState, OnPossessed := HandleProgress }
        spawn. Watcher.Start()

    HandleProgress(GameState:game_state, PlayerID:string, Agent:agent)<suspends>:void=
        if (G := GuardPropertyID?). GameState.AfterTrue(ResourceID, G, PlayerID)

        PlayAnim := play_anim_fn. Prop := ProgressBar

        spawn. GameState.SyncFloat(ResourceID, DurationPropertyID, PlayerID, UpdateAnimation)
        GameState.OnUpdated(ResourceID, StartPropertyID, PlayerID, PlayAnim.FN)
