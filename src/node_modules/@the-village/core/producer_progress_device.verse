...
producer_progress_device<public> := class(creative_device, tableau_device):
    ...
    @editable GuardPropertyID:string = ""
    @editable DurationPropertyID:string = ""
    @editable ProgressBar:creative_prop = creative_prop{}
    VFX:particle_system = TycoonPack.Particles.Money.P_CoinBurst_5

    Init<override>(GameState:game_state)<suspends>:void=
        Watcher := participant_watcher:
            GameState := GameState
            OnPossessed := HandleProducer
        spawn. Watcher.Start()

    HandleProducer(GameState:game_state, ParticipantID:string, Agent:agent)<suspends>:void=
        spawn. GameState.SyncFloat(ResourceID, DurationPropertyID, ParticipantID, UpdateAnimation)
        GameState.AfterTrue(ResourceID, GuardPropertyID, ParticipantID)

        loop:
            if (Anim := ProgressBar.GetAnimationController[]):
                Anim.Play()
                Anim.MovementCompleteEvent.Await()
                SpawnParticleSystem(VFX, ProgressBar.GetTransform().Translation)
                Sleep(0.0)
            else:
                break
