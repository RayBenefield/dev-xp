...
producer_pos_hud_device<public> := class(creative_device, tableau_device):
    ...
    SetupHUD(GameState:game_state, PlayerID:string):widget=
        Count := gooey_string. VM := Map(GameState.GetStrProp(ResourceID, "Count", PlayerID), CountMsg)
        Cost := gooey_string. VM := Map(GameState.GetStrProp(ResourceID, "Cost", PlayerID), CostMsg)
        Prod := gooey_string. VM := Map(GameState.GetStrProp(ResourceID, "Production Amount", PlayerID), ProdMsg)
        Buttons := array:
            responsive_button:
                DefaultText := "Work"
                Button := button_loud{}
                GooeyVisibility := GameState.GetBoolProp(ResourceID, "Can Work", PlayerID)
                GooeyEnabled := GameState.GetBoolProp(ResourceID, "Not Working", PlayerID)
                Action := GameState.GetAbility(ResourceID, "Produce", PlayerID)
            responsive_button:
                DefaultText := "Buy"
                GooeyEnabled := GameState.GetBoolProp(ResourceID, "Can Afford", PlayerID)
                Action := GameState.GetAbility(ResourceID, "Buy", PlayerID)
            responsive_button:
                DefaultText := "Hire"
                Button := button_loud{}
                GooeyVisibility := GameState.GetBoolProp(ResourceID, "Hireable", PlayerID)
        ...
        Widgets := array:
            ...
            responsive_text_block. GooeyText := GameState.GetStrProp(ResourceID, "Name", PlayerID)
