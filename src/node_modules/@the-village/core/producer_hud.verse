...
producer_pos_hud_device<public> := class(creative_device, tableau_device):
    ...
    Init<override>(GameState:game_state)<suspends>:void=
        GameState.AddHUD("Producer POS HUD", SetupHUD, HUDSettings)
    ...
    SetupHUD(GameState:game_state, PlayerID:string):widget=
        ...
        Buttons := array:
            gooey_button:
                ...
                Action := GameState.GetAbilityExec(ResourceID, "Produce", PlayerID)
            gooey_button:
                DefaultText := "Buy"
                EnabledGoo := GameState.GetBoolProp(ResourceID, "Can Afford", PlayerID)
                Action := GameState.GetAbilityExec(ResourceID, "Buy", PlayerID)
            gooey_button:
                DefaultText := "Hire"
                Button := button_loud{}
                VisibleGoo := GameState.GetBoolProp(ResourceID, "Hireable", PlayerID)
                Action := GameState.GetAbilityExec(ResourceID, "Hire", PlayerID)
        OrderedButtons := if (LeftHUD?). Buttons else. Buttons.Reverse()

        Count := StrGoo(Map(GameState.GetStrProp(ResourceID, "Count", PlayerID), CountMsg))
        Cost := StrGoo(Map(GameState.GetStrProp(ResourceID, "Cost", PlayerID), CostMsg))
        Prod := StrGoo(Map(GameState.GetStrProp(ResourceID, "Production Amount", PlayerID), ProdMsg))
        Widgets := array:
            gooey_stack_box:
