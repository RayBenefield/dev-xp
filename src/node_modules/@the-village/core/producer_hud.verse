...
producer_pos_hud_device<public> := class(creative_device, side_effect_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        WorkAction := State.GameState.GetAbilityExec(ResourceID, "Produce", State.PlayerID)
        WorkButton := LoudButton("Work", WorkAction)
        BuyAction := State.GameState.GetAbilityExec(ResourceID, "Buy", State.PlayerID)
        BuyButton := RegularButton("Buy", BuyAction)
        HireAction := State.GameState.GetAbilityExec(ResourceID, "Hire", State.PlayerID)
        ...
        array:
            ...
            PushBool("Not Working", WorkButton.SetEnabled)
            PushBool("Can Afford", BuyButton.SetEnabled)
