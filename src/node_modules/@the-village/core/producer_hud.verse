...
producer_pos_hud_device<public> := class(creative_device, side_effect_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        array:
            PushMsg("Name", SetText(NameText).FN)
            PushMsg("Cost", CostMsg, SetText(CostText).FN)
            PushMsg("Count", CountMsg, SetText(CountText).FN)
            PushMsg("Production Amount", ProdMsg, SetText(ProdText).FN)
