...
CreateProducerResource<public><constructor>(P:producer_settings) := resource:
    ...
    Props := map:
        ...
        "Has One" => prop_bool{ Focus := Path("Count"), FN := "=", Tweak := ONE() }
        "Has Some" => prop_bool{ Focus := Path("Count"), FN := ">=", Tweak := ONE() }
        "Has Workers" => prop_bool{ Focus := Path("Workers"), FN := ">=", Tweak := ONE() }
        "No Workers" => prop_bool{ Focus := Path("Workers"), FN := "=", Tweak := ZERO() }
    ...
    Abilities := map:
        "Buy" => ability. Effects := array:
            IsTrue("Can Afford")
            Decrease(P.CostResource, "Count").By("Cost")
            Increase("Count").By(ONE())
            IsTrue("Has Workers")
            Execute("Produce")
        "Produce" => ability. Effects := array:
            IsTrue("Has Some")
            IsFalse("Working")
            Set("Working").To(TRUE())
