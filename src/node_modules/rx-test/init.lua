...
local produce = function(state, args, level)
    local stream = rawget(state, OBSERVABLE_KEY)

    local onNext, onError, onComplete = observableSpy(stream)
    ...
    if (not assertCalls)then
        _.printTable(received, 'Received', truePrint)
        _.printTable(expected, 'Expected', truePrint)
    end
    ...
    for key, val in pairs(received) do
        ...
        elseif (type(expected[key]) == 'table') then
            local assertTables = _.areTablesEqual(expected[key], val)
            if (not assertTables)then
                _.printTable(val, 'Received', truePrint)
                _.printTable(expected[key], 'Expected', truePrint)
            end
...
assert:register('modifier', 'to', _.identity)
...
return function(needPrint)
    if (needPrint) then
        global('print', truePrint)
        assert:register('assertion', 'printed', function() return true end)
