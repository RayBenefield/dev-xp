...
const contentDir = proj => `${proj}/Content`

export default async ({
    manifest: { name: folder, dependencies },
    structure: { name, root, srcDir },
}) => {
    const destDir = contentDir(folder)
    ...
    const externals = map(depStructures, ({ name: ex, destDir: exDest }) => {
        const isNamespaced = dep => dep.startsWith('@') && dep.indexOf('/') > 0
        const dest = isNamespaced(ex)
            ? `${destDir}/${pascalCase(ex.substring(1, ex.indexOf('/')))}`
            : `${destDir}/${pascalCase(ex)}`
        return { src: exDest, dest }
    })
