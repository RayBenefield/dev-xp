...
import { mkdir, writeSync } from '@dev-xp/fs'
...
export const preCreate = ({ root, packagePath, name }) => ({
    ...
    manifestContents: {
        name: capitalCase(name).replaceAll(' ', '_'),
        dependencies: [
            `@${name}`,
            'tableau',
            '@tableau',
...
export default ({
    ...
    mkdir(packagePath)
    mkdir(scopedPath)
    writeSync(join(scopedPath, '_assets.verse'), '')
    writeSync(join(scopedPath, '_content.verse'), '')
    writeSync(join(scopedPath, '_devices.verse'), '')
