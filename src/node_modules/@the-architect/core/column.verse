    ...
    GetResources<override>():[]resource= array:
        resource:
            ...
            Props := map:
                ...
                "Location ID" => prop_str. DefaultFrom := option. Path("Cursor", "Location ID")
                "Partial Location ID" => prop_str{ Focus := Path("Location ID"), FN := "Append", Tweak := COMMA() }
                "Block Location ID" => prop_str{ Focus := Path("Partial Location ID"), FN := "Append", Tweak := Path("Height") }
                "Block ID" => prop_str{ Focus := Path("Block Label"), FN := "Append", Tweak := Path("Block Location ID") }
                "XY Location" => prop_vec2. DefaultFrom := option. Path("Cursor", "XY Location")
                "Blocks" => prop_strs{}
                "Height" => prop_int{ Focus := Path("Blocks"), FN := "Length" }
            Actions := map:
