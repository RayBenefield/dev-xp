...
block_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    GetResources<override>():[]resource= array:
        resource:
            Type := TypeID
            Props := map:
                "Height" => prop_int{ Focus := Path(Path("CREATOR ID"), "Blocks"), FN := "Find", Tweak := Path("ID") }
                "Z Coord" => prop_float{ Focus := Path("Height"), FN := "*", Tweak := Path("Architect Cursor", "Height Increment") }
                "Location" => prop_vec3{ Focus := Path(Path("CREATOR ID"), "XY Location"), FN := "Append", Tweak := Path("Z Coord") }
