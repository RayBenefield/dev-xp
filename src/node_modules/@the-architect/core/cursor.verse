...
architect_cursor_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        array:
            PushMsg("Current Type", With("Type").FN, SetText(TypeWidget).FN)
            PushMsg("Block Type", With("Block").FN, SetText(BlockWidget).FN)
            move{ PropertyID := "Location", Prop := CursorSettings.Prop, Time := 0.2 }
