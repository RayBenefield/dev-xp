    ...
    GetResources<override>():[]resource= array:
        resource:
            ...
            Props := map:
                ...
                "Column Type" => prop_str. Default := "Column"
                "Block Type" => prop_str. Default := "Block"
                "Height" => prop_int{ Focus := Path(Path("Column ID"), "Height") }
                "Height Increment" => prop_float. Default := 64.0
                "Column ID" => prop_str{ Focus := Path("Column Type"), FN := "Append", Tweak := Path("Cursor", "Location ID") }
                ...
                "Block ID" => prop_str{ Focus := Path("Block Type"), FN := "Append", Tweak := Path("Location ID") }
            ...
            Abilities := map:
                ...
                "Try Add Column" => ability. Effects := array:
                    ...
                    Create("Column ID").As("Column Type")
                "Try Add Block" => ability. Effects := array:
                    IsFalse("Block Exists")
                    Create("Block ID").As("Block Type")
