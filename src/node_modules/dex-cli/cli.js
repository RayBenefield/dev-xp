...
import writeWorklog from '@dexterity/worklog'
...
const configHandler = createConfigHandler({ logger })
...
configHandler
    ...
    .then(config =>
        promptSession({ logger }).then(answers => ({ config, answers }))
    )
    .then(writeWorklog({ logger }))
    .catch(err => {
