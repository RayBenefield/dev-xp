...
num<public> := class<concrete><computes>():
    ...
    Normalize<public>()<transacts>:num=
        ...
        SignedValue := if (Sgn(Value) < 0.0). -1.0 * NewValue else. NewValue
        return num{ Value := SignedValue, Exponent := NewExponent }
    ...
    Add<public>(Other:float)<transacts>:num= Add(num. Value := Other)
    ...
    Multiply<public>(Other:float)<transacts>:num=
        ...
        return Result.Normalize()
    # TODO: Add tests
    Multiply<public>(Other:num)<transacts>:num=
        Result := num{ Value := Value * Other.Value, Exponent := Exponent + Other.Exponent }
        return Result.Normalize()

    # TODO: Add tests
    Pow<public>(Other:float)<transacts>:num=
        Result := if (NewExponent := Round[Exponent * Other]):
            num{ Value := Pow(Value * 1.0, Other), Exponent := NewExponent }
        else. Self
...
operator'*'<public>(LHS:num, RHS:num)<transacts>:num = LHS.Multiply(RHS)

operator'+'<public>(LHS:num, RHS:float)<transacts>:num = LHS.Add(RHS)
operator'+'<public>(LHS:float, RHS:num)<transacts>:num = RHS.Add(LHS)
operator'*'<public>(LHS:num, RHS:int)<transacts>:num = LHS.Multiply(RHS)
operator'*'<public>(LHS:int, RHS:num)<transacts>:num = RHS.Multiply(LHS)
operator'*'<public>(LHS:num, RHS:float)<transacts>:num = LHS.Multiply(RHS)
operator'*'<public>(LHS:float, RHS:num)<transacts>:num = RHS.Multiply(LHS)
# operator'>='<public>(LHS:num, RHS:num):logic = LHS.GreaterThanOrEqual(RHS)

AddNums(LHS:num, RHS:num)<transacts>:num = LHS.Add(RHS)
(All:[]num).Sum<public>():num = All.Reduce(AddNums, num{})

ToString<public>(Number:num)<transacts>:string=
