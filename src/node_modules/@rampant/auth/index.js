...
authRouter.get('/twitter', async (req, res) => {
    ...
    const state = JSON.parse(decodeURIComponent(req.cookies.__session))
    const authUrl = await authTwitter.getAuthUrl(state)
    ...
    return axios({
        ...
        .then(({ data: credentials }) => {
            ...
            return client.helix.users
                ...
                .then(details => {
                    // eslint-disable-next-line no-underscore-dangle
                    const user = classToPlain(details)._data
                    db
                        .update('users/raybenefield', {
                            twitchId: details.id,
                            twitch: user,
                        })
                        .then(() => {
