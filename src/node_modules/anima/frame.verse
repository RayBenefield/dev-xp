...
Frame<public><constructor>()<transacts> := frame{}
Frame<public><constructor>(T:transformer)<transacts> :=
    frame. Transformers := array. T
Frame<public><constructor>(T1:transformer, T2:transformer)<transacts> :=
    frame. Transformers := array{ T1, T2 }
Frame<public><constructor>(T1:transformer, T2:transformer, T3:transformer)<transacts> :=
    frame. Transformers := array{ T1, T2, T3 }
ResetFrame<public><constructor>(Transform:transform)<transacts> := frame. Transformers := array:
    MoveTo(Transform.Translation),
    RotateTo(Transform.Rotation.ToVector3()),
    ScaleTo(Transform.Scale)
...
UseTransformer(Transformer:transformer, Final:vector3)<transacts>:vector3= Transformer.Transform(Final)
