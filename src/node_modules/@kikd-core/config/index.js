...
export default async ({
    manifest: { name: folder, dependencies: deps },
    ...
    const [include, exact] = partition(deps, isNamespace)
    const namespaced = await findPackages({ root, include })
    const packageNames = [...namespaced, ...exact]
    const depStructures = map(getStructures({ root, packageNames }), post => ({
        type: 'lua-rock',
        post,
    }))

    const post = { root, name, dependencies: packageNames, srcDir, destDir }
    return [...depStructures, { type, post }]
}
