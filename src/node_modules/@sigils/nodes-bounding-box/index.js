...
export default (editor, nodes) => {
    ...
    const right = max(
        nodes.map(
            node =>
                node.position[0] + editor.view.nodes.get(node).el.clientWidth
        )
    )
    const bottom = max(
        nodes.map(
            node =>
                node.position[1] + editor.view.nodes.get(node).el.clientHeight
        )
    )
    ...
    return {
        ...
        getCenter: () => [(left + right) / 2, (top + bottom) / 2],
    }
}
