    ...
    GetResources<override>():[]resource= array:
        resource:
            ...
            Abilities := map:
                ...
                "Water" => array:
                    ...
                    effect{ FN := "Increase", Focus := Path("Stage"), Tweak := Path("ONE") }
        ...
        resource:
            ...
            Props := map:
                ...
                "Count" => prop_num{}
                "X" => prop_num{ Focus := Path("X Coord"), FN := "Multiply", Tweak := Path("Grid Size") }
                "Y" => prop_num{ Focus := Path("Y Coord"), FN := "Multiply", Tweak := Path("Grid Size") }
                "XY Location" => prop_vec2{ Focus := Path("X Coord"), FN := "Append", Tweak := Path("Y Coord") }
                "Location" => prop_vec3{ Focus := Path("XY Location"), FN := "Append", Tweak := Path("ZERO") }
            ...
            Abilities := map:
                ...
                "Create Plot" => array:
                    ...
                    effect{ FN := "Set", Focus := Path(Path("Plot ID"), "X Coord"), Tweak := Path("X Coord")}
                    effect{ FN := "Set", Focus := Path(Path("Plot ID"), "Y Coord"), Tweak := Path("Y Coord")}
                    effect{ FN := "Increase", Focus := Path("Count"), Tweak := Path("ONE") }
                "Move Left" => array:
                    effect{ FN := "Decrease", Focus := Path("X Coord"), Tweak := Path("Grid Size") }
                "Move Right" => array:
                    effect{ FN := "Increase", Focus := Path("X Coord"), Tweak := Path("Grid Size") }
                "Move Forward" => array:
                    effect{ FN := "Decrease", Focus := Path("Y Coord"), Tweak := Path("Grid Size") }
                "Move Back" => array:
                    effect{ FN := "Increase", Focus := Path("Y Coord"), Tweak := Path("Grid Size") }
