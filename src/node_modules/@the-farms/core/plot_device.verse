...
plot_device<public> := class(creative_device, config_device, side_effect_device):
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        Plot := State.GameState.SpawnProp(DirtProp, State.Source, Path("Location"))
        Bar := State.GameState.SpawnProp(BarProp, State.Source, Path("Location"))

        if (Prop := Bar?):
            Duration := State.GetFloat("Growth Time").Get()
        ...
        array:
            toggle_prop{ PropertyID := "Has Plant", Prop := Plant }
            toggle_material{ PropertyID := "Watered", Prop := Plot, True := WetMaterial, False := DryMaterial }
            toggle_material{ PropertyID := "Is Seed", Prop := Plant, True := SeedMaterial, False := PlantMaterial }
            toggle_animation{ PropertyID := "Is Plant", Prop := Plant, True := Anims(1).Deltas(), False := Anims(0).Deltas() }
            set_mesh{ PropertyID := "Stage", Prop := Plant, AllMeshes := AllMeshes }
            play_animation{ PropertyID := "Stage", Prop := Plant }
            play_animation{ PropertyID := "Watered Count", Prop := Bar }
