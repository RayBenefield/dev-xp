...
farming_device<public> := class(creative_device, config_device, side_effect_device):
        ...
        resource:
            ...
            Abilities := map:
                "Action" => ability. Effects := array:
                    ...
                    Execute("Try Harvest")
                    Execute("Create Plot")
                "Try Water" => ability. Effects := array:
                    IsTrue("Exists")
                    IsTrue("Can Water")
                    Execute(Path("Plot ID"), "Water")
                "Try Plant" => ability. Effects := array:
                    IsTrue("Exists")
                    IsTrue("Can Plant")
                    Execute(Path("Plot ID"), "Plant")
                "Try Harvest" => ability. Effects := array:
                    IsTrue("Exists")
                    IsTrue("Can Harvest")
                    Execute(Path("Plot ID"), "Harvest")
                "Create Plot" => ability. Effects := array:
                    IsFalse("Exists")
    ...
    GetSideEffects<override>(State:scoped_state)<suspends>:[]side_effect=
        ...
        array:
            toggle_prop{ PropertyID := "Can Plant", Prop := option. PlantIcon }
            toggle_prop{ PropertyID := "Can Water", Prop := option. WaterIcon }
            toggle_prop{ PropertyID := "Can Harvest", Prop := option. HarvestIcon }
